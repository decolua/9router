# 8Router Architecture

_Last updated: 2026-02-06_

## Executive Summary

8Router is a local AI routing gateway and dashboard built on Next.js.
...
flowchart LR
    ...
    subgraph Router[8Router Local Process]
...
- `src/lib/localDb.js`
- file: `${DATA_DIR}/db.json` (or `~/.8router/db.json` when `DATA_DIR` is unset)
- entities: providerConnections, providerNodes, modelAliases, combos, apiKeys, settings, pricing

Usage DB:

- `src/lib/usageDb.js`
- files: `~/.8router/usage.json`, `~/.8router/log.txt`
...
- main state: `${DATA_DIR}/db.json` (or `~/.8router/db.json`)
- usage stats: `~/.8router/usage.json`
- request log lines: `~/.8router/log.txt`
...
flowchart LR
    ...
    subgraph ContainerOrProcess[8Router Runtime]
        Next[Next.js Server\nPORT=20128]
...
1. `usageDb` currently stores under `~/.8router` and does not follow `DATA_DIR`.
...
- Build from source: `cd /root/dev/8router && npm run build`
- Build Docker image: `cd /root/dev/8router && docker build -t 8router .`
