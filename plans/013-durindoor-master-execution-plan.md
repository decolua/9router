...
- **🔴 HIGH-PRIORITY — migration layer (user directive 2026-06-22):** `plans/015-seamless-9router-migration.md` — 100% back-compatible 9router→DurinDoor upgrade. Primary hook = `cli/hooks/postinstall.js` → unified `migrateLegacy9router()` (data dir + runtime + old service-unit/autostart detect+migrate) + back-compat READs (re-add `9router` bin redirect; tool-config keys `[providers.9router]`/`custom:9Router`/`model_providers.9router`; `sk_9router` default key; env already done). **Supersedes 4A "drop 9router bin" + 4B "manual service reinstall".** IMPLEMENT NEXT.
- **Feature workstream — Headroom metrics page (user directive 2026-06-22):** `plans/016-headroom-metrics-page.md` — fold standalone Headroom into the router + metrics/health page at the EXISTING `/dashboard/system/compress/headroom`. **Slice-0 RESOLVED: hybrid source priority (custom IP:port > auto-detected local > router-native fallback); `/api/headroom/probe` kept as detector.** ⚠️ **NATIVE FALLBACK BLOCKED** — the Headroom compression algorithm is NOT in-repo (`open-sse/rtk/headroom.js` has only the external call); needs the standalone proxy's source/spec. Feasible now: source-priority + instrumentation + page using external sources. Order: fold(1) → instrument + `/api/headroom/stats`(2) → page(3, `source` top-level).
