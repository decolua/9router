...
- **Feature workstream — Headroom metrics page (user directive 2026-06-22):** `plans/016-headroom-metrics-page.md` — fold the standalone Headroom compression proxy INTO the router + a metrics/health page at the EXISTING `/dashboard/system/compress/headroom` (replace its external-probe UI). Discovery: compression is still external-proxy (NOT folded); no in-router stats. **Mandatory order: fold (Slice 1) → instrument + read-only `/api/headroom/stats` endpoint (Slice 2) → page (Slice 3).** Slice-0 decision (embed router-native vs dual-mode) blocks implementation.
