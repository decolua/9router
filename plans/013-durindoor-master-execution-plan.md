...
- **Feature workstream — Headroom metrics page (user directive 2026-06-22):** `plans/016-headroom-metrics-page.md` — fold standalone Headroom into the router + metrics/health page at the EXISTING `/dashboard/system/compress/headroom`. **Slice-0 RESOLVED: hybrid source priority (custom IP:port > auto-detected local > router-native fallback); `/api/headroom/probe` kept as detector.** Discovery: compression still external-proxy; no in-router stats. Order: fold(1) → instrument + read-only `/api/headroom/stats`(2) → page(3, `source: custom|detected|native` top-level). PREREQ: native fallback needs the Headroom compression algorithm (verify its source first).
