...
test('deploy journal resolves timestamp pointer under global CLI directory', async()=>{ const reads=[]; const adapter=createSystemAdapter({globalCliDir:'/cli',readText:async p=>{reads.push(p);return p.endsWith('.deploy-latest')?'20260711-232049\n':'2026-07-11T23:20:49+0700 STATE DONE\n';}}); assert.equal((await adapter.deployJournal()).status,'pass'); assert.deepEqual(reads,['/cli/.deploy-latest','/cli/.deploy-journal.20260711-232049']); });
test('core adapter detects journal, git, live and PM2 failures', async()=>{ const outputs=new Map([['git rev-parse HEAD','abc\n'],['git rev-list --left-right --count HEAD...@{upstream}','0\t1\n'],['pm2 jlist','[]']]); const adapter=createSystemAdapter({expectedCommit:'abc',readText:async p=>p.endsWith('.deploy-latest')?'stamp':'2026 STATE FAILED\n',run:async(c,a)=>({stdout:outputs.get(`${c} ${a.join(' ')}`)||'',stderr:'',code:0}),fetch:async u=>new Response(u.endsWith('/api/health')?'bad':'{}',{status:200})}); assert.equal((await adapter.deployJournal()).status,'fail'); assert.equal((await adapter.gitState()).status,'fail'); assert.equal((await adapter.liveState()).status,'fail'); assert.equal((await adapter.pm2State()).status,'fail'); });
