...
describe("WindsurfExecutor class", () => {
  it("constructor wires config from fallback when provider is hidden", () => {
  ...
  it("fallback config baseUrl is the chat endpoint when provider is hidden", () => {
    const ex = new WindsurfExecutor();
    expect(ex.config.baseUrl).toBe(
