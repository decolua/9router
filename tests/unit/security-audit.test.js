...
// These assertions read repo source off disk. `path.resolve("src/...")` resolves
// against the process cwd, which is the repo root only when the suite happens to
// be launched from there — running it the documented way (`cd tests && npx
// vitest run`) made all thirteen fail with ENOENT on tests/src/... Anchor on this
// file's own location instead, so the audit runs from anywhere.
const REPO_ROOT = path.resolve(path.dirname(fileURLToPath(import.meta.url)), "../..");
const repoPath = (rel) => path.join(REPO_ROOT, rel);
