...
async function callCompress(url, messages, model, timeoutMs, compressUserMessages, diagnostics, token) {
  ...
  // Headroom rejects non-loopback callers with 401 when HEADROOM_PROXY_TOKEN is set.
  if (token) headers["X-Headroom-Proxy-Token"] = token;
  let res;
  try {
...
export async function compressWithHeadroom(body, { enabled, url, token, model, format, compressUserMessages, timeoutMs = DEFAULT_TIMEOUT_MS, diagnostics = null } = {}) {
