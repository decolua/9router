 ...
 */
export async function handleForcedSSEToJson({ providerResponse, sourceFormat, targetFormat, provider, model, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, onUsage, customToolNames, trackDone, appendLog, reqTag, log }) {
  ...
  if (isCodexResponsesApi) {
    try {
      ...
      if (onUsage) Promise.resolve(onUsage(usage, finalBody)).catch((error) => console.warn(`[API key quota] usage callback failed: ${error.message}`));
