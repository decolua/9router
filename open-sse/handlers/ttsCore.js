...
export async function handleTtsCore({ provider, model, input, credentials, responseFormat = "mp3", language }) {
  ...
  try {
    ...
    return createErrorResult(HTTP_STATUS.BAD_GATEWAY, err.message || "TTS synthesis failed", err.resetsAtMs, {
      errorCode: err.errorCode,
      upstreamStatus: err.isUpstreamError ? err.status : null,
      isUpstreamError: err.isUpstreamError,
    });
  }
}
