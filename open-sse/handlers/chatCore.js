...
import { resolveThinkingSynthesis } from "../utils/stream.js";
...
export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, sourceFormatOverride, providerThinking, requestedModel, comboSystemPrompt, comboThinkingUsage }) {
  ...
  // Per-request hidden-thinking synthesis decision (per-combo mode + ratio
  // drawn ONCE here): the response seams below reuse the same { enabled,
  // ratio } so every client-facing usage in this request reports the same
  // share. Account-fallback/token-refresh retries re-enter handleChatCore and
  // re-draw — only the successful attempt's numbers reach the client.
  const thinkingSynthesis = resolveThinkingSynthesis(body, model, clientThinkingIntent, comboThinkingUsage);
  ...
  // Final wire format of the body just before dispatch.
  ...
  // Transform flags accumulator for the single "⚙" log line below.
  const xf = [];

  // Combo identity system prompt: grounds the model in the combo's client-visible
  // name (Explicit Identity Grounding) ahead of the client's own system content.
  if (comboSystemPrompt) {
    injectSystemPrompt(translatedBody, finalFormat, comboSystemPrompt, { position: "prepend" });
    xf.push("IDENTITY");
  }

  if (xf.length && log?.line) log.line(reqTag, "⚙", xf.join(" · "));

  // Pin cache breakpoints to the final body — the identity-prompt injection above
  // can reshape system content, and a stale anchor costs a full prefix rewrite.
  ...
  const sharedCtx = { provider, model, body, stream, translatedBody, finalBody, requestStartTime, connectionId, apiKey, clientRawRequest, onRequestSuccess, requestedModel, reqTag, log, thinkingIntent: clientThinkingIntent, thinkingSynthesis };
