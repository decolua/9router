...
import { getModelTargetFormat, getModelTransportFormat, getModelStrip, getModelUpstreamId, getModelType, PROVIDER_ID_TO_ALIAS } from "../config/providerModels.js";
...
export async function handleChatCore({ body, modelInfo, credentials, log, onCredentialsRefreshed, onRequestSuccess, onDisconnect, clientRawRequest, connectionId, userAgent, apiKey, ccFilterNaming, rtkEnabled, headroomEnabled, headroomUrl, headroomCompressUserMessages, cavemanEnabled, cavemanLevel, ponytailEnabled, ponytailLevel, pxpipeEnabled, pxpipeMinChars, pxpipeTimeoutMs, pxpipeTransform, onPxpipeEvent, sourceFormatOverride, providerThinking, requestedModelOverride }) {
  const { provider, model } = modelInfo;
  const requestedModel = typeof requestedModelOverride === "string" && requestedModelOverride.trim()
  ...
  // A model-level transport is authoritative when an upstream assigns different
  // models to different API endpoints (OpenCode Go). Otherwise retain the normal
  // source-format passthrough behavior for multi-dialect providers.
  const modelTransportFormat = getModelTransportFormat(alias, model);
  const runtimeTransport = resolveTransport(provider, sourceFormat, modelTransportFormat);
  ...
  const actualModel = upstreamModel || model;
  ...
  try {
    ...
    trackPendingRequest(model, provider, connectionId, false, true);
    appendRequestLog({ ...requestLogContext, status: `FAILED ${error.name === "AbortError" ? 499 : HTTP_STATUS.BAD_GATEWAY}` }).catch(() => { });
