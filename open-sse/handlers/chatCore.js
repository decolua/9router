...
export async function handleChatCore({
	...
	// keys defensively — this is the passthrough safety net (chatCore does
	// not strip _toolNameMap / _clientSessionId for passthrough branches,
	// and we don't want a future translator bug to leak them upstream).
	if (VALIDATE_OUTBOUND) {
		...
		if (!validation.ok) {
			...
			console.log(`${COLORS.red}[VALIDATE] ${errMsg}${COLORS.reset}`);
			log?.warn?.(
				"VALIDATE",
			...
			appendRequestLog({
				...
				provider,
				connectionId,
				status: `FAILED ${HTTP_STATUS.BAD_REQUEST}`,
			}).catch(() => {});
			saveRequestDetail(
				buildRequestDetail({
					provider,
	...
	// Defensive strip AFTER the gate — only reachable when validation passed.
	// If a translator ever stops removing _toolNameMap / _clientSessionId, the
	// gate above fires first and short-circuits with 400. This strip is a
	// belt-and-suspenders pass for unknown underscore keys (silent) and for
	// the case where VALIDATE_OUTBOUND is disabled.
