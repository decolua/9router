...
function applyFormat(fmt, body, cfg, caps, isCodex = false) {
  ...
  switch (fmt) {
    case "openai": {
      ...
      // OpenAI reasoning_effort enum caps at "xhigh" (no "max"): clamp "max"→"xhigh".
      // Codex accepts "max" natively, so preserve xhigh/max on that surface.
      if (level) body.reasoning_effort = (!isCodex && level === "max") ? "xhigh" : level;
      break;
    }
