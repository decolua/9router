...
import type { JsonValue } from "../types/executor.js";
...
export const INTERNAL_KEYS: readonly string[] = Object.freeze([
	"_toolNameMap",
	"_clientSessionId",
]);
...
const OPENAI_ROLES = new Set<string>([
	ROLE.USER,
	ROLE.ASSISTANT,
	ROLE.TOOL,
	ROLE.SYSTEM,
...
const CLAUDE_ROLES = new Set<string>([ROLE.USER, ROLE.ASSISTANT]);
const GEMINI_ROLES = new Set<string>([GEMINI_ROLE.USER, GEMINI_ROLE.MODEL]);
const CLAUDE_BLOCK_TYPES = new Set<string>([
...
const OPENAI_CONTENT_TYPES = new Set<string>([
	OPENAI_BLOCK.TEXT,
...
export function stripInternalKeys(body: Body): Body {
...
function validateOpenAI(body: Body, errors: ValidationError[]): void {
	if (
		body.model === null ||
	...
	(body.messages as JsonValue[]).forEach((msg: JsonValue, i: number) => {
		const p = `messages[${i}]`;
		...
		if (Array.isArray(m.content)) {
			(m.content as JsonValue[]).forEach((block: JsonValue, j: number) => {
				if (!block || typeof block !== "object") return;
				const b = block as Body;
				if (b.type && !OPENAI_CONTENT_TYPES.has(b.type as string)) {
					pushError(
						errors,
						`${p}.content[${j}].type`,
	...
	if (Array.isArray(body.tools)) {
		(body.tools as JsonValue[]).forEach((tool: JsonValue, i: number) => {
			const p = `tools[${i}]`;
...
function validateClaude(body: Body, errors: ValidationError[]): void {
	if (
		body.model === null ||
		body.model === undefined ||
		(typeof body.model !== "string" && typeof body.model !== "object")
		...
		(body.messages as JsonValue[]).forEach((msg: JsonValue, i: number) => {
			...
			if (Array.isArray(m.content)) {
				(m.content as JsonValue[]).forEach((block: JsonValue, j: number) => {
					if (!block || typeof block !== "object") return;
					const b = block as Body;
					if (b.type && !CLAUDE_BLOCK_TYPES.has(b.type as string)) {
						pushError(
							errors,
							`${p}.content[${j}].type`,
	...
	if (body.system != null) {
		if (typeof body.system !== "string" && !Array.isArray(body.system)) {
			...
			(body.system as JsonValue[]).forEach((block: JsonValue, i: number) => {
				if (
					!block ||
					typeof block !== "object" ||
					((block as Body).type && (block as Body).type !== "text")
				) {
					pushError(
						errors,
						`system[${i}]`,
...
function validateGemini(body: Body, errors: ValidationError[]): void {
	if (
		body.model === null ||
		body.model === undefined ||
		(typeof body.model !== "string" && typeof body.model !== "object")
	...
	(root.contents as JsonValue[]).forEach((msg: JsonValue, i: number) => {
...
function validateOpenAIResponses(body: Body, errors: ValidationError[]): void {
	if (
		body.model === null ||
		body.model === undefined ||
		(typeof body.model !== "string" && typeof body.model !== "object")
...
export function validateOutboundPayload(
	...
	for (const k of Object.keys(b)) {
		if ((INTERNAL_KEYS as string[]).includes(k)) {
