  ...
  display: {
    ...
    notice: {
      text:
        ...
        "SSO / profile (recommended): leave the API key empty and set " +
        'providerSpecificData {"profile":"my-sso-profile","region":"us-east-1"}, then run ' +
        "`aws sso login --profile my-sso-profile` — credentials refresh automatically. " +
        "Static keys: use the AWS secret access key as the API key and set " +
        'providerSpecificData {"accessKeyId":"AKIA…","region":"us-east-1"}, plus ' +
        '"sessionToken" if they are temporary (ASIA…) keys.',
      apiKeyUrl:
