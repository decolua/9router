 ...
 */
export function reconstructBodyForResume(originalBody, textBuffer, provider, model, sourceFormat, targetFormat, credentials, connectionId, clientTool) {
  ...
  if (Array.isArray(newBody.messages)) {
    ...
    if (provider !== "anthropic" && provider !== "kiro" && provider !== "openrouter") {
    ...
    if (lastContent && lastContent.role === "model") {
      if (lastContent.parts && lastContent.parts[0]) {
        lastContent.parts[0].text = (lastContent.parts[0].text || "") + accumulated;
      }
    } else {
 ...
 */
export async function executeResumeRequest({ originalBody, textBuffer, provider, model, credentials, sourceFormat, targetFormat, userAgent, apiKey, connectionId, toolNameMap, reqLogger, clientRawRequest }) {
  dbg("RESUME", `Initiating transparent stream resume | provider=${provider} | model=${model} | bytesGenerated=${textBuffer.totalContentLength}`);
  
  const clientTool = userAgent?.toLowerCase().includes("droid") ? "droid" : "other";
  const newBody = reconstructBodyForResume(originalBody, textBuffer, provider, model, sourceFormat, targetFormat, credentials, connectionId, clientTool);
  if (!newBody) {
  ...
  try {
    ...
    if (!result.response.ok) {
      dbg("RESUME", `Resume request failed with status: ${result.response.status}`);
      return null;
    }
