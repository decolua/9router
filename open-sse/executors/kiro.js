      ...
      async transform(chunk, controller) {
        ...
        while (buffer.length >= 16 && iterations < maxIterations) {
          ...
          if (eventType === "assistantResponseEvent" && event.payload?.content) {
            ...
            if (thinkingExpected) {
              // Claude on Kiro emits reasoning inline when the system prompt
              // enables thinking. Split it into the OpenAI reasoning_content
              // channel so downstream consumers see the same shape they would
              // get from a native reasoning model.
