...
import { ensureSession, finishRun, gateFromText, preflight, pruneSessionState, requestSession, resetSessionCache, sessionFailureHandler, sessionStateSize, SESSION_STALE_CODES, startRun } from "./freebuffSession.js";
...
export class FreebuffExecutor extends BaseExecutor {
  ...
  async execute({ model, body, stream, credentials, signal, log, proxyOptions = null }) {
    ...
    const poolId = selectedFreebuffPoolId(proxyOptions); preflight(token, model, proxyOptions, poolId); const handleSessionFailure = sessionFailureHandler({ token, model, proxyOptions, log });
    ...
    try { runId = await startRun(token, proxyOptions, rootAgentIdForModel(model)); activeRunId = runId; let result = await doChat(); if (SESSION_STALE_CODES.has(result.response.status)) { const gate = gateFromText(await result.response.text().catch(() => "")); if (gate.kind !== "stale") { finish("cancelled"); return handleSessionFailure(new Error(JSON.stringify({ error: gate.kind === "limited_ip" ? "session_model_mismatch" : "model_locked", message: gate.kind === "limited_ip" ? "limited IP" : "", currentModel: gate.currentModel })), signal); } finish("cancelled"); try { session = await ensureSession(token, model, proxyOptions, true); runId = await startRun(token, proxyOptions, rootAgentIdForModel(model)); activeRunId = runId; } catch (error) { return handleSessionFailure(error, signal); } result = await doChat(); } finish(result.response.ok ? "completed" : "failed"); return { ...result, url: this.buildUrl(), headers: this.buildHeaders(credentials, stream) }; } finally { if (activeRunId) finishRun(token, activeRunId, "failed", proxyOptions); }
  }
}
