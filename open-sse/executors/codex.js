...
function normalizeCodexTools(body) {
    ...
    body.tools = body.tools.filter((tool) => {
        ...
        if (type !== "function") {
            if (!type || tool.function || typeof tool.name === "string")
                return false;
    ...
    if (Array.isArray(body?.input) && body.input.length > 0) {
        ...
        if (text.length >= MIN_LEN) {
            const hash = hashContent(
                (machineId || "") + text.slice(0, CAP_LEN),
            );
            ...
            ? args.body.input.reduce(
                  (n, it) =>
                      ...
                      (Array.isArray(it.content)
                          ? it.content.filter((c) => c.type === "image_url")
                                .length
                          : 0),
                  0,
              )
    ...
    async _peekSseOverloaded(response) {
        ...
        try {
            while (text.length < CODEX_SSE_PEEK_BYTES) {
                ...
                const hit = CODEX_SSE_OVERLOADED_PATTERNS.find((p) =>
                    text.includes(p),
                );
                if (hit) {
                    matched = hit;
