...
`handlers/chatCore.js` → `services/model.js` `parseModel` (resolve `provider/model`) → **pre-translate hooks** (`rtk/systemInject.js` identity/system-prompt inject — fail-open) → `executors/index.js` `getExecutor(provider)` → `translator/index.js` `translateRequest` (client format → provider format) → `executor.execute()` (streams upstream) → `translateResponse` (provider chunks → client format) → SSE out.
...
- `rtk/` — `systemInject.js` shared identity/system-prompt injector (used by the combo identity-prompt feature).
...
- `rtk/systemInject.js` mutates the request body in-place and is **fail-open**: any error returns null and leaves the body untouched — never throw out of it.
