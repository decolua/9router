...
export const PROVIDERS = {
  ...
  kiro: {
    ...
    format: "kiro",
    // 429 is handled by the aggressive `kiroRateLimit` path below; other
    // transient statuses still use the shared retry config.
    retry: { 502: 3, 503: 3 },
    ...
    // waits (which aligns our retries with everyone else's), we poll freed
    // capacity on a short, *jittered* exponential backoff so we win the race
    // for a freed slot more often. Honors upstream Retry-After when present.
    //
    // Two profiles, selected per-request by the user's Kiro mode setting
    // (providerStrategies.kiro.kiroMode):
