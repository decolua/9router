...
## Breaking Changes
- **CLI tools**: remove the CLI Tools install/config dashboard (`/dashboard/cli-tools`) and its config-writer API (`/api/cli-tools/*`), plus the shared MCP marketplace (McpMarketplaceModal + `cowork-mcp-registry`/`cowork-mcp-tools`) that lived under it. Deletes 51 files and drops the `confbox` and `selfsigned` dependencies. CLI coding agents keep routing through the OpenAI-compatible endpoint (`/v1/*`) — only the dashboard auto-write of their local config files is gone (configure manually). The runtime MCP stdio bridge (`/api/mcp/[plugin]/*`) and its `LOCAL_STDIO_PLUGINS` catalog are unaffected
- **MITM**: remove the MITM HTTPS-interception feature entirely (DNS hijack of Antigravity/Kiro/Copilot/Cursor hosts to a local `:443` proxy via a self-signed Root CA installed into OS trust stores). Deletes `src/mitm/`, the dashboard MITM page/API routes, the `mitmAlias` kv scope + MITM settings keys (DB migration 004, SCHEMA_VERSION 7→8), the MITM DNS-bypass in `open-sse/utils/proxyFetch.js`, and the `node-forge` dependency. Normal outbound env-proxy and provider routing are unaffected
