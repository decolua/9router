...
export function geminiCLIUserAgent(model = "unknown") {
  ...
  return `GeminiCLI/${GEMINI_CLI_VERSION}/${model || "unknown"} (${osName}; ${arch()})`;
}
...
export function getPlatformEnum(): number {
  const osName = platform();
  const architecture = arch();
  if (osName === "darwin") return architecture === "arm64" ? PLATFORM.DARWIN_ARM64 : PLATFORM.DARWIN_AMD64;
  if (osName === "linux") return architecture === "arm64" ? PLATFORM.LINUX_ARM64 : PLATFORM.LINUX_AMD64;
  if (osName === "win32") return PLATFORM.WINDOWS_AMD64;
