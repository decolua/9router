...
const v = (...keys: (keyof typeof VOICES)[]) => keys.map((k) => ({ ...VOICES[k], type: "tts" }));
...
export const TTS_MODELS_CONFIG: Record<string, TtsProviderConfig> = {
  ...
  "google-tts": {
    defaults: GOOGLE_TTS_LANGUAGES as Voice[],
  },
};

// ── Helper: get voices for a specific model ────────────────────────────────
export function getTtsVoicesForModel(provider: string, modelId: string): Voice[] | null {
...
export function buildTtsProviderModels(): Record<string, Voice[]> {
  const entries: Record<string, Voice[]> = {};
