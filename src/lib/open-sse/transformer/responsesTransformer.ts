...
export interface ResponsesLogger {
  logInput: (event: string) => void;
  logOutput: (event: string) => void;
  flush: () => void;
}

// Create log directory for responses (Node.js only)
export function createResponsesLogger(model: string, logsDir: string | null = null): ResponsesLogger | null {
 ...
 */
export function createResponsesApiTransformStream(logger: ResponsesLogger | null = null): TransformStream {
  const state: any = {
    seq: 0,
  ...
  const startReasoning = (controller: TransformStreamDefaultController, idx: number) => {
  ...
  const closeMessage = (controller: TransformStreamDefaultController, idx: string | number) => {
  ...
  const closeToolCall = (controller: TransformStreamDefaultController, idx: string | number) => {
