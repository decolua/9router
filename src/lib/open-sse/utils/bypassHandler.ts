 ...
 */
export function handleBypassRequest(body: any, model: string, userAgent: string = "", ccFilterNaming: boolean = false): BypassResult | null {
  ...
  if (!shouldBypass && (SKIP_PATTERNS as any)?.length) {
    const userMessages = messages.filter(m => m.role === "user");
    const userText = userMessages.map(m => getText(m.content)).join(" ");
    if ((SKIP_PATTERNS as any).some((p: string) => userText.includes(p))) {
      shouldBypass = true;
    }
  }
  ...
  const sourceFormat: any = detectFormat(body);
 ...
 */
function createOpenAIResponse(model: string, text: string = DEFAULT_BYPASS_TEXT): any {
 ...
 */
function createNonStreamingResponse(sourceFormat: string, model: string, text?: string): BypassResult {
  ...
  const state: any = initState(sourceFormat);
  state.model = model;
  ...
  for (const chunk of openaiChunks) {
    const translated: any = translateResponse(FORMATS.OPENAI, sourceFormat, chunk, state);
  ...
  // Flush remaining
  const flushed: any = translateResponse(FORMATS.OPENAI, sourceFormat, null, state);
  if (flushed?.length > 0) {
...
/**
 * Merge translated chunks into final response object
 */
function mergeChunksToResponse(chunks: any[], sourceFormat: string): any {
 ...
 */
function createOpenAIStreamingChunks(completeResponse: any): any[] {
