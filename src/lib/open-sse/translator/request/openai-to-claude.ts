...
export function openaiToClaudeRequest(model: string, body: any, stream: boolean) {
...
function getContentBlocksFromMessage(msg: any, toolNameMap: Map<string, string> = new Map()) {
  const blocks: any[] = [];
...
function openaiToClaudeRequestForAntigravity(model: string, body: any, stream: boolean) {
  ...
  if (result.system && Array.isArray(result.system)) {
    result.system = result.system.filter((block: any) =>
