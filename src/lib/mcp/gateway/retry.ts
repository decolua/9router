 ...
 * @param opts       { baseDelayMs, backoffFactor, jitterRatio, maxDelayMs }
 * @returns          delay in milliseconds
 */
function calculateDelay(attempt: number, opts: RetryOpts = {}): number {
 ...
 *
 * @param fn                 async function to retry; () => Promise<T>
 * @param opts               retry policy options
 * @param opts.maxAttempts   total attempts (initial + retries); default 3
 * @param opts.baseDelayMs   base delay; default 100ms
 * @param opts.backoffFactor backoff multiplier; default 2
 * @param opts.jitterRatio   jitter ±%; default 0.25 (25%)
 * @param opts.maxDelayMs    max single delay; default 2000ms
 * @param opts.isTransient   (err) => boolean; defaults to common network/timeout checks
 * @param opts.onRetry       (err, attempt, delayMs) => void; called before each retry sleep
 * @returns {Promise<T>}
 */
...
function defaultIsTransient(err: unknown): boolean {
  ...
  if (e["name"] === "McpAuthError") return false;
