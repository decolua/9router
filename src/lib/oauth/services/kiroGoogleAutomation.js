...
// names, and aria-label fallbacks for English + Indonesian.
...
const APPROVE_BUTTON_SELECTORS = [
  ...
  // Google Workspace specific selectors (Commits 9-10)
  '#confirm',
...
export async function runGoogleAccountAutomation({
  ...
  while (Date.now() - startTime < shortTimeoutMs) {
    ...
    const nextEmailInput = await waitForFirstVisibleLocator(
      page,
