  ...
  try {
    const mitmPidFile = path.join(
      process.platform === "win32"
        ? path.join(process.env.APPDATA || "", "zcus-9router")
        : path.join(os.homedir(), ".zcus-9router"),
      "mitm",
      ".mitm.pid"
    );
  ...
  if (platform === "win32") {
    try {
      ...
      lines.forEach(line => {
        ...
        const isAppProcess = lower.includes("zcus-9router") ||
    ...
    try {
      ...
      output.split("\n").forEach(line => {
        const isAppProcess = line.includes("zcus-9router") ||
...
function getDataDir() {
  ...
  if (process.platform === "win32") {
    return path.join(process.env.APPDATA || path.join(os.homedir(), "AppData", "Roaming"), "zcus-9router");
  }
  return path.join(os.homedir(), ".zcus-9router");
}
