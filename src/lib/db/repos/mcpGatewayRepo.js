 ...
 *   @returns {Promise<string[]>}  list of instanceId
 */
 ...
 *   @returns {Promise<Array<{instanceId: string, toolAllowlist: string[]|null}>>}
 */
export async function getGrantsForKeyDetailed(keyId) {
  ...
  return rows.map((r) => {
    ...
    if (r.toolAllowlist != null && r.toolAllowlist !== "") {
      try {
        const parsed = JSON.parse(r.toolAllowlist);
