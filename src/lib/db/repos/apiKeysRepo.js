...
import { DEFAULT_API_KEY_GROUP_ID, getDefaultApiKeyGroup } from "./apiKeyGroupsRepo.js";
...
export async function createApiKey(name, machineId, groupId = null) {
  ...
  const db = await getAdapter();
  if (!groupId) groupId = (await getDefaultApiKeyGroup())?.id || DEFAULT_API_KEY_GROUP_ID;
...
export async function updateApiKey(id, data) {
  ...
  const fallbackGroupId = (await getDefaultApiKeyGroup())?.id || DEFAULT_API_KEY_GROUP_ID;
  db.run(`UPDATE apiKeys SET key = ?, name = ?, machineId = ?, isActive = ?, allowedModels = ?, allowedCombos = ?, groupId = ? WHERE id = ?`, [merged.key, merged.name, merged.machineId, merged.isActive ? 1 : 0, stringifyJson(Array.isArray(merged.allowedModels) ? merged.allowedModels : []), stringifyJson(Array.isArray(merged.allowedCombos) ? merged.allowedCombos : []), merged.groupId || fallbackGroupId, id]);
