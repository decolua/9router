...
async function getApiKeyMap() {
  try {
    ...
    const map = {};
    for (const k of await getApiKeys()) map[k.key] = { name: k.name, id: k.id, createdAt: k.createdAt };
    return map;
  } catch {
    return {};
  }
...
function getApiKeyName(apiKey, apiKeyMap) {
  if (!apiKey) return "Local (No API Key)";
  return apiKeyMap[apiKey]?.name || `${apiKey.slice(0, 8)}...`;
}
  ...
  const recentRequests = [...recentRing.items]
    ...
    .map((e) => {
      ...
      return {
        ...
        apiKeyName: getApiKeyName(e.apiKey, apiKeyMap),
  ...
  const recentRequests = recentRows
    .map((r) => {
      ...
      return {
        ...
        apiKeyName: getApiKeyName(r.apiKey, apiKeyMap),
        promptTokens: t.prompt_tokens || t.input_tokens || 0,
        completionTokens: t.completion_tokens || t.output_tokens || 0,
        status: r.status || "ok",
      };
