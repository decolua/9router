...
// Stats rows store the routing key (node id for custom nodes, e.g.
// "anthropic-compatible-<uuid>"). When the node/connection is later deleted
// the id no longer resolves — surface that as a readable label instead of a
// bare uuid. Returns the name or undefined to signal "not a deleted node".
function deletedNodeLabel(provider, providerNameMap) {
  if (providerNameMap[provider]) return undefined;
  if (/^(anthropic|openai)-compatible-[0-9a-f-]{36}$/.test(provider)) {
    return "已删除的连接";
  }
  return undefined;
}
...
export async function getStatsFilters() {
  ...
  for (const r of rows) {
    ...
    if (p) {
      const dead = deletedNodeLabel(p, providerNameMap);
      providerSet.set(p, { id: p, name: providerNameMap[p] || dead || p });
    }
    if (m) modelSet.add(m);
...
// Finest-grain aggregate (provider × model × connectionId) for the dashboard
// breakdown table. Every hierarchy (provider→account→model and
// model→provider→account) rolls up from this one grain, and weighted latency
// averages survive roll-up because SUMs travel with the rows.
export async function getStatsBreakdown(filter = {}) {
  await ensureStatsBackfilled();
  const db = await getAdapter();
  const { where, params } = buildStatsWhere(filter);
  return db.all(
    `SELECT provider, model, connectionId,
            ...
            COALESCE(SUM(cachedTokens), 0) as cachedTokens,
            COALESCE(SUM(cacheCreationTokens), 0) as cacheCreationTokens,
            COALESCE(SUM(latencyTotal), 0) as latencySum,
            COALESCE(SUM(latencyTtft), 0) as ttftSum
     FROM requestStats ${where}
