...
export interface Settings {
  ...
  providerThinking?: Record<string, any>;
  tunnelDashboardAccess: boolean;
  observabilityEnabled: boolean;
  enableObservability?: boolean; // Alias for backward compatibility or different naming in code
...
const DB_FILE = isCloud ? "" : path.join(DATA_DIR, "db.json");
...
function ensureDbShape(data: any): { data: DbData; changed: boolean } {
  ...
  return { data: next as DbData, changed };
}
...
async function safeRead(db: Low<DbData>) {
  await withFileLock(() => db.read());
}

async function safeWrite(db: Low<DbData>) {
  await withFileLock(() => db.write());
}
...
export async function getDb(): Promise<Low<DbData>> {
  if (isCloud) {
    if (!dbInstance) {
      ...
      dbInstance = new Low({ read: async () => data, write: async () => { } }, data);
  ...
  if (!dbInstance) {
    dbInstance = new Low(new JSONFile<DbData>(DB_FILE), cloneDefaultData());
  }
...
export async function createProviderNode(data: Partial<ProviderNode>) {
  const db = await getDb();
...
export async function updateProviderNode(id: string, data: Partial<ProviderNode>) {
  const db = await getDb();
  if (!db.data.providerNodes) db.data.providerNodes = [];
...
export async function getProxyPools(filter: { isActive?: boolean; testStatus?: string } = {}) {
  ...
  return pools.sort((a, b) => new Date(b.updatedAt || 0).getTime() - new Date(a.updatedAt || 0).getTime());
}
...
export async function createProxyPool(data: Partial<ProxyPool>) {
  const db = await getDb();
...
export async function updateProxyPool(id: string, data: Partial<ProxyPool>) {
  const db = await getDb();
  if (!db.data.proxyPools) db.data.proxyPools = [];
...
export async function createProviderConnection(data: any) {
  ...
  const optionalFields: (keyof ProviderConnection)[] = [
  ...
  for (const field of optionalFields) {
    if (data[field] !== undefined && data[field] !== null) {
      (connection as any)[field] = data[field];
    }
  }
...
export async function updateProviderConnection(id: string, data: Partial<ProviderConnection>) {
  const db = await getDb();
...
export async function setMitmAliasAll(toolName: string, mappings: Record<string, string>) {
  const db = await getDb();
...
export async function createCombo(data: Partial<Combo>) {
  const db = await getDb();
...
export async function cleanupProviderConnections() {
  const db = await getDb();
  const fieldsToCheck: (keyof ProviderConnection)[] = [
    "displayName", "email", "globalPriority", "defaultModel",
    "accessToken", "refreshToken", "expiresAt", "tokenType",
    "scope", "idToken", "projectId", "apiKey", "testStatus",
  ...
  for (const connection of db.data.providerConnections) {
    for (const field of fieldsToCheck) {
      if (connection[field] === null || connection[field] === undefined) {
        delete (connection as any)[field];
        cleaned++;
      }
    }
...
export async function importDb(payload: any) {
...
export async function getPricing() {
  ...
  for (const [provider, models] of Object.entries(PROVIDER_PRICING)) {
    ...
    if (userPricing[provider]) {
      for (const [model, pricing] of Object.entries(userPricing[provider] as Record<string, any>)) {
        merged[provider][model] = merged[provider][model]
          ? { ...merged[provider][model], ...(pricing as any) }
          : pricing;
      }
    }
  }
  ...
  for (const [provider, models] of Object.entries(userPricing)) {
    if (!merged[provider]) {
      ...
      for (const [model, pricing] of Object.entries(models as any)) {
...
export async function updatePricing(pricingData: Record<string, Record<string, any>>) {
  const db = await getDb();
