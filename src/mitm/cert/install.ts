...
function checkCertInstalledWindows(): Promise<boolean> {
  return new Promise((resolve) => {
    ...
    exec("certutil -store Root \"8Router MITM Root CA\"", { windowsHide: true }, (error) => {
      resolve(!error);
    });
  });
}
 ...
 */
export async function installCert(sudoPassword: string | null | undefined, certPath: string): Promise<void> {
...
async function installCertMac(sudoPassword: string | null | undefined, certPath: string): Promise<void> {
  // Remove all old certs with same name first to avoid duplicate/stale cert conflict
  const deleteOld = `security delete-certificate -c "8Router MITM Root CA" /Library/Keychains/System.keychain 2>/dev/null || true`;
 ...
 */
export async function uninstallCert(sudoPassword: string | null | undefined, certPath: string): Promise<void> {
  const isInstalled = await checkCertInstalled(certPath);
  if (!isInstalled) {
...
async function uninstallCertMac(sudoPassword: string | null | undefined, certPath: string): Promise<void> {
  const fingerprint = getCertFingerprint(certPath).replace(/:/g, "");
...
async function uninstallCertWindows(): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    exec(
      `certutil -delstore Root "8Router MITM Root CA"`,
      { windowsHide: true },
      (error) => {
...
function checkCertInstalledLinux(): Promise<boolean> {
  const certFile = `${LINUX_CERT_DIR}/8router-root-ca.crt`;
  return Promise.resolve(fs.existsSync(certFile));
}

async function installCertLinux(sudoPassword: string | null | undefined, certPath: string): Promise<void> {
  if (!isSudoAvailable()) {
    log(`🔐 Cert: cannot install to system store without sudo — trust this file on clients: ${certPath}`);
    return;
  }
  const destFile = `${LINUX_CERT_DIR}/8router-root-ca.crt`;
...
async function uninstallCertLinux(sudoPassword: string | null | undefined): Promise<void> {
  if (!isSudoAvailable()) {
    return;
  }
  const destFile = `${LINUX_CERT_DIR}/8router-root-ca.crt`;
