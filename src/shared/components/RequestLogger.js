...
export default function RequestLogger() {
  ...
  return (
    <div className="flex flex-col gap-4" data-i18n-skip>
      ...
      <Modal isOpen={!!selectedLog} onClose={() => { setSelectedLog(null); setLogDetail(null); }} title="流量日志详情" footer={<Button variant="ghost" onClick={() => { setSelectedLog(null); setLogDetail(null); }}>关闭</Button>}>
        {detailLoading ? <div className="py-10 text-center text-text-muted">正在加载详情...</div> : <div className="flex max-h-[65vh] flex-col gap-4 overflow-y-auto"><div className="grid grid-cols-2 gap-3 text-xs sm:grid-cols-3"><Detail label="时间" value={selectedLog?.timestamp ? new Date(selectedLog.timestamp).toLocaleString("zh-CN") : "-"} /><Detail label="状态" value={selectedLog?.status || "-"} /><Detail label="提供商" value={selectedLog?.provider || "-"} /><Detail label="模型" value={selectedLog?.actualModel || selectedLog?.model || "-"} /><Detail label="路由最终模型" value={selectedLog?.routerSelectedModel || "-"} /><Detail label="路由最终提供商" value={selectedLog?.routerSelectedProvider || "-"} /></div>{!logDetail ? <p className="rounded-md border border-dashed border-border p-4 text-sm text-text-muted">未找到对应的请求详情记录。请确认已启用请求详情记录。</p> : <><DetailBlock title="上游回传信息" value={logDetail.providerResponse || logDetail.response || {}} /><DetailBlock title="请求结果" value={logDetail.response || {}} /></>}</div>}
      </Modal>
    </div>
  );
}

function Detail({ label, value }) { return <div className="rounded-md bg-bg-subtle px-3 py-2"><div className="text-text-muted">{label}</div><div className="mt-1 break-all font-medium text-text-main">{value}</div></div>; }
function DetailBlock({ title, value }) { return <div><h3 className="mb-2 text-sm font-semibold">{title}</h3><pre className="max-h-72 overflow-auto rounded-md bg-bg-subtle p-3 text-xs leading-5 text-text-main">{typeof value === "string" ? value : JSON.stringify(value, null, 2)}</pre></div>; }
