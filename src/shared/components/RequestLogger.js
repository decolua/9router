        ...
        <div className="p-0 overflow-x-auto max-h-[600px] overflow-y-auto font-mono text-xs">
          {loading && logs.length === 0 ? (
            <div className="p-8 text-center text-text-muted">
              {i18nText("Loading logs...")}
            </div>
          ) : logs.length === 0 ? (
            <div className="p-8 text-center text-text-muted">
              {i18nText("No logs recorded yet.")}
            </div>
          ) : (
            <table className="w-full text-left border-collapse whitespace-nowrap">
              <thead className="sticky top-0 bg-bg-subtle border-b border-border z-10">
                <tr>
                  ...
                  <th className="px-3 py-2">{i18nText("Status")}</th>
                </tr>
              </thead>
                ...
                {logs.map((log, i) => {
                  ...
                  return (
                    ...
                    >
                      ...
                      <td
                        className={`px-3 py-1.5 font-bold ${isSuccess ? "text-success" : isFailed ? "text-error" : "text-primary animate-pulse"}`}
                      >
                        {status}
                      </td>
                    </tr>
