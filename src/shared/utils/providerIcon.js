...
// Dynamic connection ids (e.g. "openai-compatible-chat-<uuid>") never have a
// static icon file under /public/providers — skip the request entirely so no
// 404 ever hits the console for them.
...
/** Resolve icon file id (after alias). Empty if unknown/dynamic/no icon. */
export function resolveProviderIconId(providerId) {
  const id = normalizeId(providerId);
  if (!id) return "";
  if (failedIds.has(id)) return "";
  if (UUID_SUFFIX_RE.test(id)) return "";
