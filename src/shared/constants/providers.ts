...
export function isOpenAICompatibleProvider(providerId: string) {
  return typeof providerId === "string" && providerId.startsWith(OPENAI_COMPATIBLE_PREFIX);
}

export function isAnthropicCompatibleProvider(providerId: string) {
  return typeof providerId === "string" && providerId.startsWith(ANTHROPIC_COMPATIBLE_PREFIX);
}

// All providers (combined)
export const AI_PROVIDERS: Record<string, AiProvider> = { ...FREE_PROVIDERS, ...FREE_TIER_PROVIDERS, ...OAUTH_PROVIDERS, ...APIKEY_PROVIDERS } as Record<string, AiProvider>;
...
export function getProviderByAlias(alias: string): AiProvider | null {
...
export const ALIAS_TO_ID: Record<string, string> = Object.values(AI_PROVIDERS).reduce((acc: Record<string, string>, p: AiProvider) => {
  acc[p.alias] = p.id;
  return acc;
}, {});

// ID to Alias mapping
export const ID_TO_ALIAS: Record<string, string> = Object.values(AI_PROVIDERS).reduce((acc: Record<string, string>, p: AiProvider) => {
  acc[p.id] = p.alias;
  return acc;
}, {});
...
export function getProvidersByKind(kind: string): AiProvider[] {
  return Object.values(AI_PROVIDERS).filter((p: AiProvider) => {
