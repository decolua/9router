 ...
 */
function matchPattern(pattern: string, model: string) {
  const regex = new RegExp("^" + pattern.split("*").map((s: string) => s.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")).join(".*") + "$");
 ...
 * @returns {object|null}
 */
export function getPricingForModel(provider: string | null | undefined, model: string) {
  if (!model) return null;

  // 1. Provider-specific override
  if (provider && (PROVIDER_PRICING as any)[provider]?.[model]) {
    return (PROVIDER_PRICING as any)[provider][model];
  }

  // 2. Canonical model pricing (strip vendor prefix if needed: "deepseek/deepseek-chat" → "deepseek-chat")
  const baseModel = model.includes("/") ? model.split("/").pop()! : model;
  if ((MODEL_PRICING as any)[baseModel]) return (MODEL_PRICING as any)[baseModel];
  if ((MODEL_PRICING as any)[model]) return (MODEL_PRICING as any)[model];
 ...
 */
export function calculateCostFromTokens(tokens: any, pricing: any) {
