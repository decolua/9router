...
function CallbackContent() {
  ...
  useEffect(() => {
    ...
    const expectedOrigins = [
      ...
      // Same origin (for most providers)
      ...
      <div className="text-center p-8 max-w-md">
        {status === "processing" && (
          <>
            ...
            <p className="text-text-muted">
              {i18nText("Please wait while we complete the authorization.")}
            </p>
          </>
        )}
        ...
        {(status === "success" || status === "done") && (
          <>
            ...
            <h1 className="text-xl font-semibold mb-2">
              {i18nText("Authorization Successful!")}
            </h1>
            <p className="text-text-muted">
              {status === "success"
                ? i18nText("This window will close automatically...")
                : i18nText("You can close this tab now.")}
            </p>
          </>
        )}
      ...
      fallback={
        <div className="min-h-screen flex items-center justify-center bg-bg">
          <div className="text-center p-8">
            ...
            <p className="text-text-muted">{i18nText("Loading...")}</p>
          </div>
        </div>
      }
    >
