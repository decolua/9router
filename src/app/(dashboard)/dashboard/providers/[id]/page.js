...
export default function ProviderDetailPage() {
	...
	useEffect(() => {
		...
		fetch("/api/key-groups")
			...
			.then((data) => {
				if (data) setKeyGroups(data.groups || []);
			})
			.catch(() => {});
	...
	const handleSaveApiKey = async (formData) => {
		setAddConnectionError("");
		const { groupId, ...connectionFormData } = formData;
		try {
			const res = await fetch("/api/providers", {
				method: "POST",
				headers: { "Content-Type": "application/json" },
				body: JSON.stringify({ provider: providerId, ...connectionFormData }),
			});
			...
			if (res.ok) {
				if (groupId && (data?.connection?.id || data?.connection?._id)) {
					const connId = data.connection.id || data.connection._id;
					const group = keyGroups.find((g) => g.id === groupId);
					if (group) {
						const updated = [...(group.allowedConnectionIds || []), connId];
						await fetch(`/api/key-groups/${groupId}`, {
							method: "PUT",
							headers: { "Content-Type": "application/json" },
						...
						setKeyGroups((prev) =>
							prev.map((g) =>
								g.id === groupId ? { ...g, allowedConnectionIds: updated } : g,
							),
						);
					}
				}
