...
// Cached tokens are a subset of input tokens, so the hit rate is their share
// of input — plain client-side division, no extra server field needed.
const cacheHitRatio = (stats) => (stats.totalPromptTokens > 0 ? (stats.totalCachedTokens || 0) / stats.totalPromptTokens : null);
...
export default function OverviewCards({ stats }) {
  return (
    <div className="grid min-w-0 grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-6 sm:gap-4">
      ...
      <Card className="flex min-w-0 flex-col gap-1 px-4 py-3">
        <span className="text-text-muted text-sm uppercase font-semibold">Cache Hits</span>
        {cacheHitRatio(stats) === null ? (
          <span className="text-2xl font-bold text-text-muted">—</span>
        ) : (
          <span className="truncate text-2xl font-bold text-info">{(cacheHitRatio(stats) * 100).toFixed(1)}%</span>
        )}
      </Card>
      <Card className="flex min-w-0 flex-col gap-1 px-4 py-3">
