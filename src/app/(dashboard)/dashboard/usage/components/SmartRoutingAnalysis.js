    ...
    <div className="flex min-w-0 flex-col gap-5" data-i18n-skip>
      ...
      <Card className="min-w-0">
        ...
        {loading ? <div className="flex h-[420px] items-center justify-center text-text-muted">正在加载...</div> : !data?.configured ? <Empty text="请先在智能路由提供商配置中启用提供商并选择其 API 密钥。" /> : !data?.series?.length ? <Empty text="当前时间范围内没有配置主力模型的智能路由请求。" /> : (
          <div className="h-[440px] min-w-0"><ResponsiveContainer width="100%" height="100%"><LineChart data={chart} margin={{ top: 12, right: 18, left: 4, bottom: 12 }}><CartesianGrid strokeDasharray="3 3" stroke="var(--border)" /><XAxis dataKey="label" minTickGap={32} tick={{ fontSize: 11, fill: "var(--text-muted)" }} /><YAxis tickFormatter={(value) => `$${Number(value).toFixed(4)}`} tick={{ fontSize: 11, fill: "var(--text-muted)" }} width={78} /><Tooltip formatter={(value) => `$${Number(value).toFixed(8)}`} /><Legend wrapperStyle={{ fontSize: 12, lineHeight: "20px" }} />{data.series.flatMap((series, index) => { const color = COLORS[index % COLORS.length]; const actualLabel = `${series.routedProviderName}/${series.model} 真实`; const simulatedLabel = `${series.primaryModelName} 主力模拟`; return [<Line key={`${series.id}:actual`} type="monotone" dataKey={`${series.id}:actual`} name={actualLabel} stroke={color} strokeWidth={2} dot={false} />, <Line key={`${series.id}:simulated`} type="monotone" dataKey={`${series.id}:simulated`} name={simulatedLabel} stroke={color} strokeWidth={2} strokeDasharray="7 5" dot={false} />]; })}</LineChart></ResponsiveContainer></div>
        )}
      </Card>

      {!!data?.series?.length && <Card><h2 className="mb-3 text-lg font-semibold">模型对照</h2><div className="overflow-x-auto"><table className="w-full min-w-[720px] text-sm"><thead className="border-b border-border text-left text-xs text-text-muted"><tr><th className="px-3 py-2">智能路由提供商</th><th className="px-3 py-2">路由模型</th><th className="px-3 py-2">主力模型</th><th className="px-3 py-2 text-right">请求数</th></tr></thead><tbody>{data.series.map((series) => <tr key={series.id} className="border-b border-border/60"><td className="px-3 py-2">{series.providerName}</td><td className="px-3 py-2 font-mono">{series.routedProviderName}/{series.model}</td><td className="px-3 py-2 font-mono">{series.primaryModelName}</td><td className="px-3 py-2 text-right tabular-nums">{series.requests}</td></tr>)}</tbody></table></div></Card>}
    </div>
  );
}
