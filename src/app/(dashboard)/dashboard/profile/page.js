...
export default function ProfilePage() {
  ...
  const updateOutboundProxy = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        ...
        setProxyStatus({
          type: "success",
          message: i18nText("Proxy settings applied"),
        });
      } else {
        setProxyStatus({
          type: "error",
      ...
      setProxyStatus({
        type: "error",
        message: i18nText("An error occurred"),
      });
    } finally {
  ...
  const testOutboundProxy = async () => {
    ...
    if (!proxyUrl) {
      setProxyStatus({
        type: "error",
        message: i18nText("Please enter a Proxy URL to test"),
      });
      return;
    }
  ...
  const handlePasswordChange = async (e) => {
    ...
    if (passwords.new !== passwords.confirm) {
      setPassStatus({
        type: "error",
        message: i18nText("Passwords do not match"),
      });
      return;
    }
    setPassLoading(true);
    ...
    try {
      ...
      if (res.ok) {
        setPassStatus({
          type: "success",
          message: i18nText("Password updated successfully"),
        });
        setPasswords({
          current: "",
          new: "",
  ...
  const handleExportDatabase = async () => {
    ...
    try {
      ...
      setDbStatus({
        type: "success",
        message: i18nText("Database backup downloaded"),
      });
    } catch (err) {
      setDbStatus({
        type: "error",
  ...
  const handleImportDatabase = async (event) => {
    ...
    try {
      ...
      setDbStatus({
        type: "success",
        message: i18nText("Database imported successfully"),
      });
    } catch (err) {
      setDbStatus({
        type: "error",
      ...
      <div className="flex flex-col gap-6">
        ...
        <Card>
            ...
            <div className="flex items-center gap-4">
              ...
              <div>
                ...
                <p className="text-text-muted">
                  {i18nText("Running on your machine")}
                </p>
              </div>
            </div>
          ...
          <div className="flex flex-col gap-3 pt-4 border-t border-border">
            <div className="flex items-center justify-between p-3 rounded-lg bg-bg border border-border">
              <div>
                <p className="font-medium">{i18nText("Database Location")}</p>
                <p className="text-sm text-text-muted font-mono">
                  {i18nText("~/.9router/db.json")}
                </p>
              </div>
            </div>
        ...
        <Card>
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-lg font-semibold">{i18nText("Security")}</h3>
          </div>
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-medium">{i18nText("Require login")}</p>
            ...
            {settings.requireLogin === true && (
              ...
              >
                {settings.hasPassword && (
                  <div className="flex flex-col gap-2">
                    <label className="text-sm font-medium">
                      {i18nText("Current Password")}
                    </label>
                    <Input
                      type="password"
                      placeholder={i18nText("Enter current password")}
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div className="flex flex-col gap-2">
                    <label className="text-sm font-medium">
                      {i18nText("New Password")}
                    </label>
                    <Input
                      type="password"
                      placeholder={i18nText("Enter new password")}
                      value={passwords.new}
                      onChange={(e) =>
                        setPasswords({
                          ...passwords,
                  ...
                  <div className="flex flex-col gap-2">
                    <label className="text-sm font-medium">
                      {i18nText("Confirm New Password")}
                    </label>
                    <Input
                      type="password"
                      placeholder={i18nText("Confirm new password")}
        ...
        <Card>
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-lg font-semibold">
              {i18nText("Routing Strategy")}
            </h3>
          </div>
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-medium">{i18nText("Round Robin")}</p>
                <p className="text-sm text-text-muted">
                  {i18nText("Cycle through accounts to distribute load")}
                </p>
              </div>
              <Toggle
            ...
            {settings.fallbackStrategy === "round-robin" && (
              <div className="flex items-center justify-between pt-2 border-t border-border/50">
                <div>
                  <p className="font-medium">{i18nText("Sticky Limit")}</p>
                  <p className="text-sm text-text-muted">
                    {i18nText("Calls per account before switching")}
                  </p>
                </div>
                <Input
                  type="number"
        ...
        <Card>
          <div className="flex items-center gap-3 mb-4">
            ...
            <h3 className="text-lg font-semibold">{i18nText("Network")}</h3>
          </div>

          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-medium">{i18nText("Outbound Proxy")}</p>
                <p className="text-sm text-text-muted">
                  {i18nText(
            ...
            {settings.outboundProxyEnabled === true && (
              ...
              >
                <div className="flex flex-col gap-2">
                  <label className="font-medium">{i18nText("Proxy URL")}</label>
                  <Input
                ...
                <div className="flex flex-col gap-2 pt-2 border-t border-border/50">
                  <label className="font-medium">{i18nText("No Proxy")}</label>
                  <Input
                    placeholder={i18nText("localhost,127.0.0.1")}
        ...
        <Card>
          ...
          <div className="flex flex-col gap-4">
            <div className="flex items-center justify-between">
              <div>
                <p className="font-medium">
                  {i18nText("Enable Observability")}
                </p>
                <p className="text-sm text-text-muted">
                  {i18nText("Turn request detail recording on/off globally")}
                </p>
              </div>
              <Toggle
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-medium">{i18nText("Max Records")}</p>
                  <p className="text-sm text-text-muted">
                    {i18nText(
              ...
              <div className="flex items-center justify-between">
                <div>
                  <p className="font-medium">{i18nText("Batch Size")}</p>
                  <p className="text-sm text-text-muted">
                    {i18nText(
              ...
              <p className="text-xs text-text-muted italic pt-2 border-t border-border/50">
                {i18nText("Current: Keeps")}
                {settings.observabilityMaxRecords || 1000}
                {i18nText("records, batches every")}
                {settings.observabilityBatchSize || 20}
                {i18nText("requests, max")}
                {settings.observabilityMaxJsonSize || 1024}
                {i18nText("KB per field")}
              </p>
            </div>
          </div>
        </Card>
        ...
        <div className="text-center text-sm text-text-muted py-4">
          ...
          <p className="mt-1">
            {i18nText("Local Mode - All data stored on your machine")}
          </p>
        </div>
      </div>
    </div>
