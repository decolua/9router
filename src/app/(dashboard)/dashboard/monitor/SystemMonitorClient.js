...
export default function SystemMonitorClient() {
  ...
  const diskTotalBytes = Number(system?.disk?.totalBytes || 0);
  const diskUsedBytes = Number(system?.disk?.usedBytes || 0);
  const diskUsage = system?.disk?.available && diskTotalBytes > 0 ? diskUsedBytes / diskTotalBytes * 100 : null;
  const diskColor = diskUsage == null
    ? "text-text-muted"
    : diskUsage >= 90
      ? "text-red-500"
      : diskUsage >= 80
