...
const TUNNEL_BENEFITS = [
  {
    icon: "public",
    title: i18nText("Access Anywhere"),
    desc: i18nText("Use your API from any network"),
  },
  {
    icon: "group",
    title: i18nText("Share Endpoint"),
    desc: i18nText("Share URL with team members"),
  },
  {
    icon: "code",
    title: i18nText("Use in Cursor/Cline"),
    desc: i18nText("Connect AI tools remotely"),
  },
  {
    icon: "lock",
    title: i18nText("Encrypted"),
    desc: i18nText("End-to-end TLS via Cloudflare"),
  },
];
...
export default function APIPageClient({ machineId }) {
  ...
  const handleEnableTunnel = async () => {
    ...
    const progressSteps = [
      ...
      {
        delay: 5000,
        msg: "Starting cloudflared...",
      },
      {
        delay: 15000,
        msg: "Establishing connections...",
      },
      {
        delay: 30000,
        msg: "Waiting for tunnel ready...",
      },
    ];
    ...
    try {
      ...
      if (res.ok) {
        ...
        setTunnelStatus({
          type: "success",
          message: i18nText("Tunnel connected!"),
        });
      } else {
        setTunnelStatus({
          type: "error",
  ...
  const handleDisableTunnel = async () => {
    ...
    setTunnelStatus(null);
    try {
      const res = await fetch("/api/tunnel/disable", {
        method: "POST",
      });
      const data = await res.json();
      if (res.ok) {
        ...
        setTunnelStatus({
          type: "success",
          message: i18nText("Tunnel disabled"),
        });
        setShowDisableModal(false);
      } else {
        setTunnelStatus({
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <Card>
        ...
        {tunnelStatus && (
          <div
            className={`mt-3 p-2 rounded text-sm ${tunnelStatus.type === "success" ? "bg-green-500/10 text-green-600 dark:text-green-400" : tunnelStatus.type === "warning" ? "bg-yellow-500/10 text-yellow-600 dark:text-yellow-400" : "bg-red-500/10 text-red-600 dark:text-red-400"}`}
          >
      ...
      <Card>
        <div className="flex items-center justify-between mb-4">
          <h2 className="text-lg font-semibold">{i18nText("API Keys")}</h2>
        ...
        <div className="flex items-center justify-between pb-4 mb-4 border-b border-border">
          <div>
            <p className="font-medium">{i18nText("Require API key")}</p>
            <p className="text-sm text-text-muted">
              {i18nText("Requests without a valid key will be rejected")}
            </p>
          </div>
          <Toggle
        ...
        {keys.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-text-main font-medium mb-1">
              {i18nText("No API keys yet")}
            </p>
            <p className="text-sm text-text-muted mb-4">
              {i18nText("Create your first API key to get started")}
            </p>
            <Button icon="add" onClick={() => setShowAddModal(true)}>
              {i18nText("Create Key")}
            </Button>
      ...
      >
        <div className="flex flex-col gap-4">
          <Input
            label={i18nText("Key Name")}
            value={newKeyName}
            onChange={(e) => setNewKeyName(e.target.value)}
            placeholder={i18nText("Production Key")}
          />
          <div className="flex gap-2">
            <Button
      ...
      <Modal
        isOpen={!!createdKey}
        title={i18nText("API Key Created")}
        onClose={() => setCreatedKey(null)}
      >
        <div className="flex flex-col gap-4">
          <div className="bg-yellow-50 dark:bg-yellow-900/20 border border-yellow-200 dark:border-yellow-800 rounded-lg p-4">
            <p className="text-sm text-yellow-800 dark:text-yellow-200 mb-2 font-medium">
              {i18nText("Save this key now!")}
            </p>
      ...
      >
        <div className="flex flex-col gap-4">
          ...
          <p className="text-sm text-text-muted">
            {i18nText("Are you sure you want to disable the tunnel?")}
          </p>
