...
export default function APIPageClient({ machineId }) {
  ...
  return (
    <div className="flex flex-col gap-8">
      ...
      <Card className={cloudEnabled ? "" : ""}>
        <div className="flex items-center justify-between mb-4">
          <div>
            ...
            <p className="text-sm text-text-muted">{cloudEnabled ? "Using Cloud Proxy" : "Using Local Server"}</p>
          </div>
      ...
      <Card>
        ...
        {keys.length === 0 ? (
          <div className="text-center py-8">
            <span className="material-symbols-outlined text-4xl text-text-muted mb-2">vpn_key</span>
              ...
              <tbody>
                {keys.map((key) => (
                  <tr key={key.id} className="border-b border-border hover:bg-sidebar/30">
                    ...
                    <td className="p-3">
                      <div className="flex items-center gap-2">
                        <span className="font-mono text-xs text-text-muted">{key.key}</span>
                        <Button
                          size="sm"
                          variant="ghost"
                    ...
                    <td className="p-3 text-sm text-text-muted">{new Date(key.createdAt).toLocaleDateString()}</td>
                    <td className="p-3">
                      <Button
                        size="sm"
                        variant="ghost"
      ...
      {/* Cloud Enable Modal */}
      <Modal isOpen={showCloudModal} title="Enable Cloud Proxy" onClose={() => setShowCloudModal(false)}>
        <div className="flex flex-col gap-4">
          <div className="bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-800 rounded-lg p-4">
            <p className="text-sm text-blue-800 dark:text-blue-200 font-medium mb-2">What you will get</p>
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={() => setShowCloudModal(false)} variant="ghost" fullWidth disabled={cloudSyncing}>
              Cancel
            </Button>
          </div>
        </div>
      ...
      >
        <div className="flex flex-col gap-4">
          <div className="bg-red-50 dark:bg-red-900/20 border border-red-200 dark:border-red-800 rounded-lg p-4">
            <div className="flex items-start gap-3">
              ...
              <div>
                ...
                <p className="text-sm text-red-700 dark:text-red-300">All auth sessions will be deleted from cloud.</p>
              </div>
            </div>
          </div>
          ...
          <div className="flex gap-2">
            ...
            <Button onClick={() => setShowDisableModal(false)} variant="ghost" fullWidth disabled={cloudSyncing}>
              Cancel
            </Button>
          </div>
        </div>
