	...
	const handleCreateKey = async () => {
		...
		try {
			...
			if (res.ok) {
				...
				if (newKeyGroupId && data.id) {
					await fetch(`/api/keys/${data.id}`, {
						method: "PUT",
						headers: { "Content-Type": "application/json" },
						body: JSON.stringify({ groupId: newKeyGroupId }),
					});
				}
				...
				<div className="flex flex-col gap-4">
					...
					<div className="flex flex-col gap-1">
						...
						<select
							...
							className="w-full rounded-lg border border-border bg-bg-input px-3 py-2 text-sm text-text-main focus:outline-none focus:ring-2 focus:ring-primary"
						>
