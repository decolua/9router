...
export default function DefaultToolCard({
    ...
    return (
      <div className="mt-2 flex items-center gap-2">
        ...
        <button
          ...
          className={`shrink-0 px-3 py-2 rounded-lg border text-sm transition-colors ${hasActiveProviders ? "bg-bg-secondary border-border text-text-main hover:border-primary cursor-pointer" : "opacity-50 cursor-not-allowed border-border"}`}
        >
  ...
  const renderGuideSteps = () => {
    if (!tool.guideSteps)
      return (
        <p className="text-text-muted text-sm">{i18nText("Coming soon...")}</p>
      );
    return (
