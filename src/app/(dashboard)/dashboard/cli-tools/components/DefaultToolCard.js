...
export default function DefaultToolCard({ toolId, tool, isExpanded, onToggle, baseUrl, apiKeys, activeProviders = [], cloudEnabled = false, tunnelEnabled = false }) {
  ...
  const renderGuideSteps = () => {
    if (!tool.guideSteps) return <p className="text-text-muted text-sm">Setup guide for {tool.name} is not yet available. Use your DurinDoor base URL and API key from Settings to configure this tool manually.</p>;
