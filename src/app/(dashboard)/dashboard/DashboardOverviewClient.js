...
export default function DashboardOverviewClient() {
  ...
  const modelRows = useMemo(() => Object.entries(stats.byModel || {}).map(([name, item]) => ({ name: item.rawModel || name.split(" (")[0], requests: item.requests || 0, tokens: (item.promptTokens || 0) + (item.cachedTokens || 0) + (item.cacheCreationTokens || 0) + (item.completionTokens || 0), cost: item.cost || 0 })).sort((a, b) => b.tokens - a.tokens).slice(0, 7), [stats.byModel]);
  ...
  return <div className="flex min-w-0 flex-col gap-4" data-i18n-skip>
    ...
    <div className="grid grid-cols-1 gap-4 xl:grid-cols-[minmax(0,0.95fr)_minmax(0,1.05fr)]">
      <Card padding="sm" className="min-w-0"><div className="mb-3 flex items-center justify-between"><h3 className="font-semibold">模型分布</h3><span className="text-xs text-text-muted">按 Token 排序</span></div>{pieData.length ? <div className="grid items-center gap-3 lg:grid-cols-[190px_minmax(0,1fr)]"><div className="relative h-[220px]"><ResponsiveContainer width="100%" height="100%"><PieChart><Pie data={pieData} dataKey="tokens" nameKey="name" innerRadius={58} outerRadius={88} paddingAngle={3} stroke="none">{pieData.map((item, index) => <Cell key={item.name} fill={COLORS[index % COLORS.length]} />)}</Pie><Tooltip content={<ChartTooltip />} /></PieChart></ResponsiveContainer><div className="pointer-events-none absolute inset-0 flex flex-col items-center justify-center"><span className="text-2xl font-semibold">{pieData.length}</span><span className="text-xs text-text-muted">主要模型</span></div></div><div className="min-w-0"><div className="grid grid-cols-[minmax(0,1fr)_64px_76px] gap-2 border-b border-border pb-2 text-xs text-text-muted"><span>模型</span><span className="text-right">Token</span><span className="text-right">成本</span></div>{modelRows.map((item, index) => <div key={item.name} className="grid grid-cols-[minmax(0,1fr)_64px_76px] items-center gap-2 border-b border-border/50 py-2 last:border-0"><span className="flex min-w-0 items-center gap-2"><span className="size-2 shrink-0 rounded-full" style={{ backgroundColor: COLORS[index % COLORS.length] }} /><span className="truncate font-mono text-xs" title={item.name}>{item.name}</span></span><span className="text-right text-xs tabular-nums">{fmt(item.tokens)}</span><span className="text-right text-xs font-medium tabular-nums text-emerald-600">{money(item.cost)}</span></div>)}</div></div> : <EmptyChart icon="donut_large" text="当前范围内暂无模型使用数据" />}</Card>

      <Card padding="sm" className="min-w-0"><div className="mb-3 flex items-center justify-between"><h3 className="font-semibold">Token 使用趋势</h3><span className="text-xs text-text-muted">{periodLabel}</span></div>{hasTokenTrend ? <ResponsiveContainer width="100%" height={300}><AreaChart data={data.chart} margin={{ top: 8, right: 8, left: 0, bottom: 0 }}><defs><linearGradient id="dashboardTokens" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#06b6d4" stopOpacity={0.28}/><stop offset="90%" stopColor="#06b6d4" stopOpacity={0.02}/></linearGradient></defs><CartesianGrid vertical={false} strokeDasharray="4 4" strokeOpacity={0.14}/><XAxis dataKey="label" axisLine={false} tickLine={false} tickMargin={10} minTickGap={32} tick={{ fontSize: 11 }}/><YAxis axisLine={false} tickLine={false} tickMargin={8} tickFormatter={fmt} tick={{ fontSize: 11 }} width={58}/><Tooltip content={<ChartTooltip />} /><Area type="monotone" dataKey="tokens" name="Token" stroke="#06b6d4" strokeWidth={2.5} fill="url(#dashboardTokens)" activeDot={{ r: 4, strokeWidth: 0 }} /></AreaChart></ResponsiveContainer> : <EmptyChart text="当前范围内暂无 Token 使用数据" />}</Card>
    </div>

    <Card padding="sm" className="min-w-0"><div className="mb-3 flex items-center justify-between"><h3 className="font-semibold">密钥使用趋势</h3><span className="text-xs text-text-muted">显示用量最高的密钥</span></div>{hasKeyTrend ? <ResponsiveContainer width="100%" height={290}><LineChart data={data.keyChart.data} margin={{ top: 8, right: 12, left: 0, bottom: 0 }}><CartesianGrid vertical={false} strokeDasharray="4 4" strokeOpacity={0.14}/><XAxis dataKey="label" axisLine={false} tickLine={false} tickMargin={10} minTickGap={30} tick={{ fontSize: 11 }}/><YAxis axisLine={false} tickLine={false} tickMargin={8} tickFormatter={fmt} tick={{ fontSize: 11 }} width={58}/><Tooltip content={<ChartTooltip />} />{data.keyChart.series.map((series, index) => <Line key={series.id} type="monotone" dataKey={series.id} name={series.label} stroke={COLORS[index % COLORS.length]} strokeWidth={2.25} dot={false} activeDot={{ r: 4, strokeWidth: 0 }} />)}</LineChart></ResponsiveContainer> : <EmptyChart icon="key" text="当前范围内暂无密钥使用数据" />}</Card>
  </div>;
}
