...
import { Card, Button, Input, Modal, Badge, CardSkeleton, ConfirmModal } from "@/shared/components";
...
export default function KeyGroupsPageClient() {
  ...
  const handleKeyGroupChange = async (keyId, groupId) => {
    try {
      ...
      if (res.ok) {
        setKeys((prev) =>
          prev.map((k) => (k.id === keyId ? { ...k, groupId: groupId || null } : k))
        );
      }
    } catch (err) {
  ...
  const connectionById = (id) => connections.find((c) => String(c._id) === String(id));
  ...
  return (
    <div className="flex flex-col gap-8">
        ...
        {groups.length === 0 ? (
          <div className="text-center py-12">
            ...
            <p className="text-sm text-text-muted mb-4">
              Create a group to control which provider connections an API key can use
            </p>
            <Button icon="add" onClick={openCreate}>
              Create Group
            </Button>
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 flex-wrap">
                      ...
                      <Badge variant="info" icon="cable">
                        {allowedIds.length} connection{allowedIds.length !== 1 ? "s" : ""}
                      </Badge>
      ...
      <Card>
        ...
        <p className="text-sm text-text-muted mb-4">
          Assign each API key to a group to restrict which provider connections it can use.
        </p>

        {keys.length === 0 ? (
          <div className="text-center py-8">
            <p className="text-text-muted text-sm">No API keys found. Create keys on the Endpoint page.</p>
          </div>
        ) : (
            ...
            {keys.map((key) => (
              ...
              >
                <div className="flex-1 min-w-0">
                  ...
                  <code className="text-xs text-text-muted font-mono">{maskKey(key.key)}</code>
                </div>
                <div className="shrink-0 w-48">
                  <select
                    value={key.groupId ?? ""}
                    onChange={(e) => handleKeyGroupChange(key.id, e.target.value)}
      ...
      <Modal
        ...
        onClose={() => { setShowEditModal(false); setEditingGroup(null); resetForm(); }}
      >
        <GroupForm
          formName={formName}
          formDescription={formDescription}
          ...
          onToggleConnection={toggleConnectionId}
          onSubmit={handleEdit}
          onCancel={() => { setShowEditModal(false); setEditingGroup(null); resetForm(); }}
...
function GroupForm({
  ...
  return (
    <div className="flex flex-col gap-4">
      ...
      <div>
        <p className="text-sm font-medium text-text-main mb-2">Allowed Connections</p>
        {connections.length === 0 ? (
          <p className="text-sm text-text-muted">No provider connections available.</p>
        ) : (
            ...
            {connections.map((conn) => {
              ...
              return (
                ...
                >
                  ...
                  />
                  <span className="flex-1 text-sm text-text-main">{conn.name}</span>
                  <span className="text-xs text-text-muted font-mono">{conn.provider}</span>
                </label>
              );
            })}
          </div>
        ...
        {formAllowedConnectionIds.length > 0 && (
          <p className="text-xs text-text-muted mt-1">
            {formAllowedConnectionIds.length} connection{formAllowedConnectionIds.length !== 1 ? "s" : ""} selected
          </p>
        )}
      </div>
