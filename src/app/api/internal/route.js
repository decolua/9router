...
function isGetUserInfoRequest(url, body) {
  if (url.searchParams.has("getUserInfo")) return true;
  return body?.method === "getUserInfo";
}

function isGetUserFreeTierStatusRequest(url, body) {
  if (url.searchParams.has("getUserFreeTierStatus")) return true;
  return body?.method === "getUserFreeTierStatus";
}
...
export async function POST(request) {
  try {
    ...
    if (isGetUserInfoRequest(url, body)) {
      return ok(buildUserInfoResult());
    }

    if (isGetUserFreeTierStatusRequest(url, body)) {
      return ok({
        canUseAmpFree: false,
    ...
    console.error("[Amp Internal API] Error:", error);
...
export async function GET(request) {
  try {
    ...
    const url = new URL(request.url);
    if (url.searchParams.has("getUserInfo")) {
      return ok(buildUserInfoResult());
    }

    if (url.searchParams.has("getUserFreeTierStatus")) {
      return ok({
        canUseAmpFree: false,
        isDailyGrantEnabled: false,
      });
