...
export async function POST(request: NextRequest) {
  const body = await request.json().catch(() => ({})) as Record<string, JsonValue>;
  const platform = os.platform();
  const isWindows = platform === "win32";
  const isBrew = platform === "darwin" && hasBrew();
  const needsPassword = !isWindows && !isBrew;

  const sudoPassword = (typeof body["sudoPassword"] === "string" ? body["sudoPassword"] : null)
    ?? getCachedPassword()
    ?? await loadEncryptedPassword()
    ?? "";

  if (needsPassword && !String(sudoPassword).trim()) {
  ...
  const stream = new ReadableStream({
    async start(controller) {
      ...
      try {
        const result = await installTailscale(sudoPassword, shortId, (msg: string) => {
