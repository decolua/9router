...
import * as _oauthServer from "@/lib/oauth/utils/server";
...
interface OauthTokensShape {
  client: { clientId: string | undefined; clientSecret: string | null | undefined; clientIdIssuedAt: number | undefined } | undefined;
  as: { token_endpoint: string | undefined; authorization_endpoint: string | undefined; registration_endpoint: string | undefined } | undefined;
  resource: string | undefined;
  scope: string | undefined;
  _lastChallenge: string | undefined;
}

// Local structural type for instance fields accessed in this route.
interface OauthInstance {
  id: string;
  url: string | undefined;
  oauthTokens: OauthTokensShape | undefined;
}

// Session record returned by getMcpSessionStatus.
interface McpPendingSession {
  codeVerifier: string;
  clientId: string;
  redirectUri: string;
  resource: string | undefined;
  ...
  tokens: { hasAccess: boolean } | undefined;
}

// ── Typed facades for server.js helpers (helpers stay .js) ───────────────────
// Typing at the import boundary; runtime values are passed through unchanged.
const registerMcpSession = _oauthServer.registerMcpSession as (opts: {
  instanceId: string; state: string; codeVerifier: string;
  redirectUri: string; resource: string | undefined; clientId: string;
}) => boolean;
const getMcpSessionStatus = _oauthServer.getMcpSessionStatus as (
  instanceId: string, state: string
) => McpPendingSession | null;
const completeMcpSession = _oauthServer.completeMcpSession as (
  instanceId: string, state: string, result: Partial<McpPendingSession>
) => boolean;
const clearMcpSession = _oauthServer.clearMcpSession as (instanceId: string, state: string) => void;

// ── Helpers ───────────────────────────────────────────────────────────────────

// Narrow McpInstance (Record<string, unknown> & { id, slug, enabled, oauth })
// to OauthInstance by reading the known fields.
function toOauthInstance(raw: McpInstance): OauthInstance {
  const oauthTokens = raw["oauthTokens"];
  return {
    id: raw.id,
    url: typeof raw["url"] === "string" ? raw["url"] : undefined,
    oauthTokens: oauthTokens && typeof oauthTokens === "object" && !Array.isArray(oauthTokens)
      ? oauthTokens as OauthTokensShape
      : undefined,
  };
}

// Boundary cast: URL has no .headers property — this preserves the original JS
// runtime behaviour (url.headers is undefined, .get(...) throws TypeError) so
// that appBase() fails exactly as the .js did when called.  Do not "fix" this
// by switching to request.headers without an explicit behaviour-change decision.
interface UrlWithHeaders extends URL {
  headers: { get(name: string): string | null };
}

function appBase(request: NextRequest) {
  // Trust the inbound Host; the callback lands on the same app.
  const url = new URL(request.url) as UrlWithHeaders;
  return `${url.protocol}//${url.headers.get("x-forwarded-host") || url.host}`;
}
...
async function ensureClient(instance: OauthInstance, request: NextRequest) {
  ...
  const meta = await discoverAuth(instance.url, { wwwAuthenticate: instance.oauthTokens?._lastChallenge }) as AuthMeta | null;
  ...
  const reg = await registerClient(meta.registration_endpoint, redirectUri) as OauthClient;
  // Persist client + as metadata so subsequent refreshes have what they need.
  const newTokens: OauthTokensShape = {
...
export async function GET(request: NextRequest, context: { params: Promise<{ id: string; action: string }> }) {
  ...
  if (action === "authorize") {
    ...
    const meta: AuthMeta = { token_endpoint: rawAs1?.token_endpoint, authorization_endpoint: rawAs1?.authorization_endpoint, registration_endpoint: rawAs1?.registration_endpoint, resource: instance.oauthTokens?.resource };
    if (!meta.authorization_endpoint) {
      // Fall back to discovery if registration succeeded but we didn't capture AS.
      const discovered = await discoverAuth(instance.url, { wwwAuthenticate: instance.oauthTokens?._lastChallenge }) as AuthMeta | null;
  ...
  if (action === "callback") {
    ...
    const meta: AuthMeta = { token_endpoint: rawAs2?.token_endpoint, authorization_endpoint: rawAs2?.authorization_endpoint, registration_endpoint: rawAs2?.registration_endpoint, resource: instance.oauthTokens?.resource };
    ...
    try {
      ...
      await storeTokens(id, {
        ...
        // OAuthTokens.refresh_token is string | undefined — original passed null here;
        // undefined is the correct TS equivalent (field omitted vs null-stored).
        refresh_token: doc.refresh_token || undefined,
        // Preserve || semantics from original
        token_type: doc.token_type || "Bearer",
        scope: doc.scope || session.scope,
        // OAuthTokens.expires_at is number | undefined — original passed null;
        // undefined is the correct TS equivalent for the absent-expiry case.
        expires_at: doc.expires_in ? Date.now() + Number(doc.expires_in) * 1000 : undefined,
        // Persist token_endpoint so refresh can find it without re-discovery.
        token_endpoint: tokenEndpoint,
        // Narrow clientSecret null→undefined: OAuthTokens.client.clientSecret is string | undefined (no null)
