...
function buildAuthorizeUrl({ authorizationEndpoint, clientId, redirectUri, codeChallenge, state, resource, scope }) {
...
async function ensureClient(instance, request) {
  if (instance.oauthTokens?.client?.clientId) {
    return { clientId: instance.oauthTokens.client.clientId, clientSecret: instance.oauthTokens.client.clientSecret || null };
  }
...
export async function GET(request, { params }) {
  ...
  if (action === "authorize") {
    ...
    const meta = instance.oauthTokens?.as || {};
  ...
  if (action === "callback") {
    ...
    try {
      ...
      await storeTokens(id, {
        ...
        expires_at: doc.expires_in ? Date.now() + Number(doc.expires_in) * 1000 : null,
      ...
      completeMcpSession(id, state, { status: "error", error: e.message });
      return new Response(renderResultPage(false, `exchange error: ${e.message}`), {
        status: 500,
        headers: { "Content-Type": "text/html; charset=utf-8" },
      });
    } finally {
