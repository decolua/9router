...
const has9RouterConfig = (globalState: Record<string, JsonValue> | null) => {
...
export async function POST(request: NextRequest) {
  try {
    ...
    const globalState: Record<string, JsonValue> = (await readJson(getGlobalStatePath())) ?? {};
    ...
    const secrets: Record<string, JsonValue> = (await readJson(getSecretsPath())) ?? {};
