...
export async function POST(request) {
  try {
    ...
    return NextResponse.json({
      success: true,
      message: "Settings applied. Restart Letta Code, then use /model to select a 9Router model.",
      needsRestart: true,
    });
  } catch (error) {
...
export async function DELETE() {
  try {
    ...
    const message = restored
      ? "9Router config removed. Your original LM Studio provider has been restored. Restart Letta Code to take effect."
      : "9Router config removed. Restart Letta Code to take effect.";
