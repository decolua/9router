...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log("Error fetching MITM aliases:", (error as Error).message);
...
export async function PUT(request: NextRequest) {
  try {
    const { tool, mappings } = await request.json() as { tool: string; mappings: Record<string, string> };
    ...
    if (!status.dnsStatus || !(status.dnsStatus as Record<string, boolean>)[tool]) {
    ...
    console.log("Error saving MITM aliases:", (error as Error).message);
