...
function normalizeMitmRouterBaseUrlInput(input: string | null | undefined) {
...
function getPassword(provided: string | null | undefined) {
  return provided || getCachedPassword() || null;
}

function requiresSudoPassword(pwd: string | null) {
...
function checkPrivilege(pwd: string | null) {
...
export async function GET() {
  try {
    ...
    console.log("Error getting MITM status:", (error as Error).message);
...
export async function POST(request: NextRequest) {
  try {
    const { apiKey, sudoPassword, mitmRouterBaseUrl, forceKillPort443 } = await request.json() as {
      apiKey?: string;
      sudoPassword?: string;
      mitmRouterBaseUrl?: string | null;
      forceKillPort443?: boolean;
    };
    ...
    if (mitmRouterBaseUrl !== undefined && mitmRouterBaseUrl !== null) {
      try {
        ...
        return NextResponse.json(
          { error: (e as Error).message || "Invalid MITM router URL" },
          { status: 400 },
        );
      }
    }
    ...
    const err = error as Error & { code?: string; portOwner?: string };
    console.log("Error starting MITM server:", err.message);
    ...
    return NextResponse.json({ error: err.message || "Failed to start MITM server" }, { status: 500 });
  }
}
...
export async function DELETE(request: NextRequest) {
  try {
    const body = await request.json().catch(() => ({}) as { sudoPassword?: string });
    const { sudoPassword } = body as { sudoPassword?: string };
    const pwd = getPassword(sudoPassword) || await loadEncryptedPassword() || "";
    ...
    console.log("Error stopping MITM server:", (error as Error).message);
    return NextResponse.json({ error: (error as Error).message || "Failed to stop MITM server" }, { status: 500 });
  }
}
...
export async function PATCH(request: NextRequest) {
  try {
    const { tool, action, sudoPassword } = await request.json() as {
      tool?: string;
      action?: string;
      sudoPassword?: string;
    };
    ...
    console.log("Error toggling DNS:", (error as Error).message);
    return NextResponse.json({ error: (error as Error).message || "Failed to toggle DNS" }, { status: 500 });
  }
}
