...
import { getConsistentMachineId as _getConsistentMachineId } from "@/shared/utils/machineId";
const getConsistentMachineId = async (salt: string) => _getConsistentMachineId(salt as never);
...
const injectAuthHeaders = async (entries: Array<Record<string, JsonValue>>) => {
  const token = await getCliToken();
  for (const e of entries) {
    if (typeof e?.["url"] === "string" && (e["url"] as string).startsWith(LOCAL_MCP_PREFIX)) {
      e["headers"] = { ...(e["headers"] as Record<string, string> | undefined ?? {}), [CLI_TOKEN_HEADER]: token };
    }
  }
  return entries;
};
...
const write1pConfig = async (cfg: Record<string, JsonValue>) => {
...
const cleanup1pLegacy = async () => {
  ...
  const mcpServers = cfg["mcpServers"] as Record<string, JsonValue>;
...
const buildLocalBridgeEntries = (localPluginNames: string[]) => {
...
const buildCustomEntries = (customPlugins: Array<{ name?: string; url?: string; transport?: string }>) => {
...
async function writeSkipApprovals(managedServers: Array<Record<string, JsonValue>>) {
  const cfgPath = path.join(getWriteRoot(), "config.json");
  let cfg: Record<string, JsonValue> = {};
  try { cfg = JSON.parse(await fs.readFile(cfgPath, "utf-8")) as Record<string, JsonValue> || {}; }
  catch (e) { if ((e as NodeJS.ErrnoException).code !== "ENOENT") return { error: (e as NodeJS.ErrnoException).code }; }
...
export async function GET() {
  try {
    ...
    const inferenceModels = config?.["inferenceModels"];
    const models = Array.isArray(inferenceModels)
      ? inferenceModels.map((m) => (typeof m === "string" ? m : (m as Record<string, JsonValue>)?.["name"])).filter(Boolean)
      : [];
    const managedMcpRaw = config?.["managedMcpServers"];
    const managedMcp = Array.isArray(managedMcpRaw) ? managedMcpRaw as Array<Record<string, JsonValue>> : [];
    ...
    const activeLocalNames = managedMcp
      .filter((m) => stdioNames.has(m["name"] as string) && typeof m["url"] === "string" && (m["url"] as string).includes("/api/mcp/"))
    ...
    const activeCustomPlugins = managedMcp
      .filter((m) => m["custom"] || (!stdioNames.has(m["name"] as string) && typeof m["url"] === "string" && (m["url"] as string).includes("/api/mcp/")))
    ...
    return NextResponse.json({
      ...
      cowork: {
        ...
        plugins: managedMcp.filter((m) => !m["custom"] && !(stdioNames.has(m["name"] as string) && typeof m["url"] === "string" && (m["url"] as string).includes("/api/mcp/"))).map((m) => {
          const keys = m["toolPolicy"] ? Object.keys(m["toolPolicy"] as Record<string, string>) : [];
          const prefix = `${m["name"] as string}-`;
...
export async function POST(request: NextRequest) {
  try {
    const { baseUrl, apiKey, models, plugins, localPlugins, customPlugins } = await request.json() as {
      baseUrl?: string;
      apiKey?: string;
      models?: string[];
      plugins?: Array<Record<string, JsonValue>>;
      localPlugins?: string[];
      customPlugins?: Array<{ name?: string; url?: string; transport?: string }>;
    };
    ...
    const newConfig: Record<string, JsonValue> = {
      ...SECURITY_RELAX,
    ...
    let skipResult = null;
    try { skipResult = await writeSkipApprovals(managedMcpServers); } catch (e) { skipResult = { error: (e as Error).message }; }
