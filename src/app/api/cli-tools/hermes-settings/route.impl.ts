...
async function restoreProfileOnDisk(profile: HermesProfile | null) {
  if (!profile) return;
  try {
    await applyProfileToDisk({ baseUrl: profile.baseUrl, model: profile.model, apiKey: profile.apiKey ?? null });
  } catch (rollbackErr) {
    console.log("Failed to restore hermes settings during rollback:", rollbackErr);
  }
}

type FileSnapshot = { exists: true; content: string } | { exists: false; content: null };

async function readOptionalFile(filePath: string) {
  try {
    return { exists: true as const, content: await fs.readFile(filePath, "utf-8") };
  } catch (err) {
    if ((err as NodeJS.ErrnoException).code === "ENOENT") {
      return { exists: false as const, content: null };
    }
    throw err;
  }
}
...
async function restoreOptionalFile(filePath: string, snapshot: FileSnapshot) {
  if (!snapshot?.exists) {
    try {
...
async function restoreHermesDiskState(snapshot: { config: FileSnapshot; env: FileSnapshot }) {
  try {
...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      installed: true,
      settings: { model },
      has9Router: looksLike9RouterConfig(model as { base_url?: string; provider?: string } | null),
...
export async function POST(request: NextRequest) {
  try {
    const { baseUrl, apiKey, model } = await request.json() as { baseUrl?: string; apiKey?: string; model?: string };
    if (!baseUrl || !model) {
    ...
    await applyProfileToDisk({ baseUrl, apiKey: apiKey ?? null, model });

    try {
      await syncActiveProfileFromLegacySettings({ baseUrl, apiKey: apiKey ?? null, model });
    } catch (storeErr) {
...
export async function DELETE() {
  try {
    ...
    try {
      ...
      await restoreProfileOnDisk((activeProfile ?? null) as HermesProfile | null);
      throw clearErr;
    }
