...
const has9RouterConfig = (auth: Record<string, JsonValue> | null) => {
  if (!auth) return false;
  const entry = (auth["openai-compatible"] ?? auth["9router"]) as Record<string, string> | undefined;
...
export async function POST(request: NextRequest) {
  try {
    ...
    const auth: Record<string, JsonValue> = (await readJson(getAuthPath())) ?? {};
    ...
    try {
      const vscode: Record<string, JsonValue> = (await readJson(getVscodeSettingsPath())) ?? {};
