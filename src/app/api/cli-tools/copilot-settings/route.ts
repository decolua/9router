...
const readConfig = async (): Promise<any[] | null> => {
  try {
...
const has8RouterConfig = (config: any): boolean => {
  ...
  return config.some((entry: any) => entry.name === "8Router");
};

const get8RouterEntry = (config: any): any | null => {
  if (!Array.isArray(config)) return null;
  return config.find((entry: any) => entry.name === "8Router") || null;
};
...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    const entry = get8RouterEntry(config);
...
// POST - Apply 8Router config to chatLanguageModels.json
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    // Replace existing 8Router entry or append
    const idx = config.findIndex((e: any) => e.name === "8Router");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
// DELETE - Remove 8Router entry from chatLanguageModels.json
export async function DELETE(): Promise<NextResponse> {
  try {
    ...
    config = config.filter((e: any) => e.name !== "8Router");
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));

    return NextResponse.json({
      success: true,
      message: "8Router removed from Copilot config",
    });
  } catch (error) {
