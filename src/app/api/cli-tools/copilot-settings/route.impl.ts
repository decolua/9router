...
const has9RouterConfig = (config: JsonValue[] | null) => {
  if (!Array.isArray(config)) return false;
  return config.some((entry) => typeof entry === "object" && entry !== null && !Array.isArray(entry) && (entry as Record<string, JsonValue>)["name"] === "9Router");
};

const get9RouterEntry = (config: JsonValue[] | null) => {
  if (!Array.isArray(config)) return null;
  return config.find((entry) => typeof entry === "object" && entry !== null && !Array.isArray(entry) && (entry as Record<string, JsonValue>)["name"] === "9Router") as Record<string, JsonValue> | null ?? null;
};
...
export async function GET() {
  try {
    ...
    const firstModel = (Array.isArray(models) && models[0] !== undefined)
      ? models[0] as Record<string, JsonValue>
      : null;
    ...
    return NextResponse.json({
      ...
      currentModel: firstModel?.["id"] ?? null,
      currentUrl: firstModel?.["url"] ?? null,
    });
  } catch (error) {
...
export async function POST(request: NextRequest) {
  try {
    const { baseUrl, apiKey, models } = await request.json() as { baseUrl?: string; apiKey?: string; models?: string[] };
    ...
    try {
      ...
      const parsed = JSON.parse(existing) as JsonValue;
    ...
    const newEntry: Record<string, JsonValue> = {
      name: "9Router",
      vendor: "azure",
      apiKey: keyToUse,
    ...
    const idx = config.findIndex((e) => typeof e === "object" && e !== null && !Array.isArray(e) && (e as Record<string, JsonValue>)["name"] === "9Router");
    if (idx >= 0) {
      config[idx] = newEntry;
    } else {
...
export async function DELETE() {
  try {
    ...
    config = config.filter((e) => !(typeof e === "object" && e !== null && !Array.isArray(e) && (e as Record<string, JsonValue>)["name"] === "9Router"));
    await fs.writeFile(configPath, JSON.stringify(config, null, 2));
