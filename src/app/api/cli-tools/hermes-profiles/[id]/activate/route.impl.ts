...
async function restorePreviousDiskState(profile: HermesProfile | null) {
  try {
    if (profile) await applyProfileToDisk({ baseUrl: profile.baseUrl, model: profile.model, apiKey: profile.apiKey ?? null });
...
export async function POST(_request: NextRequest, context: { params: Promise<{ id: string }> }) {
  ...
  try {
    ...
    try {
      ...
      await restorePreviousDiskState((previousActiveProfile ?? null) as HermesProfile | null);
      throw storeErr;
    }
