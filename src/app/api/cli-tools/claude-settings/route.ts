...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    const has8Router = !!(settings?.env?.ANTHROPIC_BASE_URL);

    return NextResponse.json({
      installed: true,
      settings: settings,
      has8Router: has8Router,
...
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    // Normalize ANTHROPIC_BASE_URL to remove trailing slashes and /v1 (Anthropic SDK automatically appends /v1/messages)
    if (env.ANTHROPIC_BASE_URL) {
      env.ANTHROPIC_BASE_URL = env.ANTHROPIC_BASE_URL.replace(/\/+$/, "").replace(/\/v1$/, "");
    }
