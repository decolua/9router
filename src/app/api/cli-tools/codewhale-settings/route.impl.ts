...
type TomlDoc = Record<string, TomlValue>;
...
const parseToml = (content: string) => {
  ...
  for (const line of lines) {
    ...
    if (keyValueMatch) {
      const k = keyValueMatch[1] ?? "";
      const v = keyValueMatch[2] ?? "";
      currentSection[k] = v;
      continue;
    }
    ...
    if (unquotedMatch) {
      const k = unquotedMatch[1] ?? "";
      currentSection[k] = (unquotedMatch[2] ?? "").trim();
    }
  }
...
const has9RouterConfig = (config: TomlDoc | null) => {
