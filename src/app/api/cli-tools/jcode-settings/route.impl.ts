...
const readConfig = async () => {
  try {
    ...
    return parseTOML(content) as Record<string, JsonValue>;
  } catch {
    return { providers: {} } as Record<string, JsonValue>;
  }
};

const has9RouterConfig = (config: Record<string, JsonValue>) => {
  if (!config || !config["providers"]) return false;

  const providers = config["providers"] as Record<string, JsonValue>;

  if (providers["9router"]) return true;

  for (const [, provider] of Object.entries(providers)) {
    const p = provider as Record<string, JsonValue>;
    if (p["base_url"] && String(p["base_url"]).includes("localhost:20128")) {
      return true;
    }
  }
...
const writeConfig = async (config: Record<string, JsonValue>) => {
  const configPath = getConfigPath();
...
export async function POST(request: NextRequest) {
  try {
    ...
    (config["providers"] as Record<string, JsonValue>)["9router"] = {
      type: "openai-compatible",
      base_url: normalizedBaseUrl,
      auth: "bearer",
      api_key_env: "JCODE_9ROUTER_API_KEY",
      env_file: "provider-9router.env",
      default_model: (models && models.length > 0 ? models[0] : undefined) ?? "cc/claude-opus-4-7",
...
export async function DELETE() {
  try {
    ...
    delete (config["providers"] as Record<string, JsonValue>)["9router"];
