...
const getOpenClawSettingsPath = () => path.join(getOpenClawDir(), "settings.json");
...
// Check if settings has 8Router config
const has8RouterConfig = (settings: any): boolean => {
  if (!settings?.models?.providers) return false;
  return !!settings.models.providers["8router"];
};
...
const readAgentModel = async (agentDir: string): Promise<string | null> => {
  try {
    ...
    const models = data?.providers?.["8router"]?.models;
...
export async function GET(): Promise<NextResponse> {
  try {
    ...
    return NextResponse.json({
      ...
      has8Router: has8RouterConfig(settings),
...
const writeAgentModels = async (agentDir: string, model: string, baseUrl: string, apiKey: string): Promise<void> => {
...
// POST - Update 8Router settings (merge with existing settings)
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    const fullModelId = `8router/${model}`;

    // Remove all old 8router/* entries from agents.defaults.models
    Object.keys(settings.agents.defaults.models)
      .filter((k) => k.startsWith("8router/"))
    ...
    Object.values(agentModels).forEach((m: any) => { if (m) allModelIds.add(m); });

    // Add fresh 8router models to allowlist
    allModelIds.forEach((m) => {
      settings.agents.defaults.models[`8router/${m}`] = {};
    });

    // Remove old 8router model from each agent in agents.list
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent: any) => {
        if (agent.model?.startsWith("8router/")) {
    ...
    // Update models.providers.8router with all models
    settings.models.providers["8router"] = {
    ...
    if (settings.agents.list) {
      settings.agents.list = settings.agents.list.map((agent: any) => {
        ...
        if (agentModel) return { ...agent, model: `8router/${agentModel}` };
        return agent;
      });

      // Write per-agent models.json for agents with agentDir
      await Promise.all(
        settings.agents.list.map(async (agent: any) => {
...
export async function DELETE(): Promise<NextResponse> {
  try {
    ...
    // Remove 8Router from models.providers
    if (settings.models && settings.models.providers) {
      delete settings.models.providers["8router"];
    ...
    // Remove 8router models from agents.defaults.models allowlist
    if (settings.agents?.defaults?.models) {
      const keysToRemove = Object.keys(settings.agents.defaults.models).filter((k) => k.startsWith("8router/"));
    ...
    // Reset agents.defaults.model.primary if it uses 8router
    if (settings.agents?.defaults?.model?.primary?.startsWith("8router/")) {
