...
export async function POST(request: NextRequest) {
  try {
    ...
    const getResult = await getResponse.json() as { success: boolean; message?: string; data: { name?: string } };
    ...
    const postResult = await postResponse.json() as { success: boolean; message?: string; data: { apiKey?: string; name?: string; expireTime?: string } };
    ...
    return NextResponse.json({
      ...
      connection: {
        ...
        apiKey: refreshedKey.apiKey ? refreshedKey.apiKey.substring(0, 10) + "..." : "", // masked
        expireTime: refreshedKey.expireTime,
      },
    });
  } catch (error) {
