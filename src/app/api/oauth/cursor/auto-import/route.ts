...
function normalizeDbValue(value: string | null): string | null {
...
function extractTokensViaBetterSqlite(dbPath: string) {
  ...
  const normalizeLocal = (value: string | null): string | null => {
    if (typeof value !== "string") return value;
    try {
      const parsed = JSON.parse(value);
      return typeof parsed === "string" ? parsed : value;
  ...
  for (const key of ACCESS_TOKEN_KEYS) {
    const raw = query(key);
    if (raw) { accessToken = normalizeLocal(raw); break; }
  }

  let machineId: string | null = null;
  for (const key of MACHINE_ID_KEYS) {
    const raw = query(key);
    if (raw) { machineId = normalizeLocal(raw); break; }
  }
...
async function extractTokensViaCLI(dbPath: string) {
  ...
  for (const key of ACCESS_TOKEN_KEYS) {
    try {
      ...
      if (raw) {
        accessToken = normalizeLocal(raw);
        break;
      }
    } catch {
      /* try next */
