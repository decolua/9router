  ...
  try {
    ...
    try {
      ...
      if (parts.length === 3) {
        ...
        if (!tokenPart) throw new Error("malformed JWT");
        const base64 = tokenPart.replace(/-/g, "+").replace(/_/g, "/");
        const missingPadding = (4 - (base64.length % 4)) % 4;
        const padded = base64 + "=".repeat(missingPadding);
        const payload = JSON.parse(Buffer.from(padded, "base64").toString("utf8")) as Record<string, JsonValue>;

        // Extract from OpenAI JWT structure
        const auth = (payload["https://api.openai.com/auth"] ?? {}) as Record<string, JsonValue>;
        const profile = (payload["https://api.openai.com/profile"] ?? {}) as Record<string, JsonValue>;
        const rawEmail = profile["email"] ?? payload["email"] ?? payload["preferred_username"] ?? null;
