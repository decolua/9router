...
// JS helper JSDoc marks codeVerifier/extraData required but runtime accepts undefined;
// boundary alias widens signature so callers can pass optional args safely.
const pollOptional = pollForToken as (
  provider: string,
  deviceCode: string,
  codeVerifier?: string,
  extraData?: JsonValue,
) => ReturnType<typeof pollForToken>;
const poll = (provider: string, deviceCode: string, codeVerifier?: string, extraData?: JsonValue) =>
  pollOptional(provider, deviceCode, codeVerifier ?? undefined, extraData ?? undefined);
...
export async function GET(request: NextRequest, context: { params: Promise<{ provider: string; action: string }> }) {
  try {
...
export async function POST(request: NextRequest, context: { params: Promise<{ provider: string; action: string }> }) {
  try {
    const { provider, action } = await context.params;
    let body;
    try {
    ...
    if (action === "exchange") {
      ...
      if (code && code.startsWith("eyJ") && code.includes(".")) {
        // Dynamic import: extractCodexAccountInfo is conditionally needed only in this branch
        ...
        let directPayload: Record<string, JsonValue> = {};
        try {
        ...
        const providerSpecificData: Record<string, JsonValue> = { authMethod: "access_token" };
    ...
    if (action === "poll") {
      ...
      if (noPkceProviders.includes(provider)) {
        result = await poll(provider, deviceCode);
      } else if (provider === "kiro") {
        // Kiro needs extraData (clientId, clientSecret) from device code response
        result = await poll(provider, deviceCode, undefined, extraData);
        ...
        result = await poll(provider, deviceCode, codeVerifier, extraData);
      } else {
        // Qwen and other PKCE providers
        if (!codeVerifier) {
          return NextResponse.json({ error: "Missing code verifier" }, { status: 400 });
        }
        result = await poll(provider, deviceCode, codeVerifier);
      }
