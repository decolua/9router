...
function normalizeProxyConfig(body: any = {}): any {
...
async function normalizeProxyPoolUpdate(proxyPoolId: any): Promise<any> {
  if (proxyPoolId === undefined) {
    return { hasProxyPoolField: false };
  }

  if (proxyPoolId === null || proxyPoolId === "" || proxyPoolId === "__none__") {
...
function shouldMergeProviderSpecificData(existing: any, incoming: any, hasLegacyProxy: boolean, hasProxyPoolField: boolean): boolean {
...
export async function PUT(request: Request, context: { params: Promise<{ id: string }> }): Promise<NextResponse> {
  try {
    ...
    const existing: any = await getProviderConnectionById(id);
    if (!existing) {
      return NextResponse.json({ error: "Connection not found" }, { status: 404 });
    }
    ...
    const updated: any = await updateProviderConnection(id, updateData);
