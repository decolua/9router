...
const OAUTH_TEST_CONFIG: Record<string, any> = {
  ...
  github: {
    ...
    authHeader: "Authorization",
    authPrefix: "Bearer ",
    extraHeaders: { "User-Agent": "8Router", "Accept": "application/vnd.github+json" },
  },
  iflow: {
...
async function refreshOAuthToken(connection: any): Promise<any> {
...
async function testOAuthConnection(connection: any, effectiveProxy: any = null): Promise<any> {
...
async function fetchWithConnectionProxy(url: string, options: any = {}, effectiveProxy: any = null): Promise<Response> {
...
async function testApiKeyConnection(connection: any, effectiveProxy: any = null): Promise<any> {
    ...
    switch (connection.provider) {
      ...
      case "alicode-intl": {
        ...
        const res = await fetchWithConnectionProxy(aliBaseUrl, {
          method: "POST",
          headers: { "Authorization": `Bearer ${connection.apiKey}`, "content-type": "application/json" },
          body: JSON.stringify({ model: getDefaultModel(connection.provider) || "qwen-turbo", max_tokens: 1, messages: [{ role: "user", content: "test" }] }),
        }, effectiveProxy);
        const valid = res.status !== 401 && res.status !== 403;
        return { valid, error: valid ? null : "Invalid API key" };
      }
 ...
 */
export async function testSingleConnection(id: string): Promise<any> {
  ...
  return { valid: result.valid, error: result.error, latencyMs, testedAt: new Date().toISOString(), refreshed: result.refreshed };
}
