...
export async function POST(request, { params }) {
  try {
    ...
    const firstResult = await pingModelByKind(`${alias}/${first.id}`, firstKind, baseUrl, { connectionId: id });
    const results = [{ modelId: first.id, name: first.name || first.id, ...firstResult }];

    if (rest.length > 0) {
      const restResults = await Promise.all(
        rest.map(async (model) => {
          const result = await pingModelByKind(`${alias}/${model.id}`, model.kind || model.type || "llm", baseUrl, { connectionId: id });
