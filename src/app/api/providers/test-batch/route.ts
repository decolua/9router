...
function getAuthGroup(providerId: string, connection: any = null): string {
  // Prioritize authType from connection if available
  if (connection?.authType) {
    if (connection.authType === "oauth") {
      // Check if it's a free provider
      if ((FREE_PROVIDERS as any)[providerId]) return "free";
      return "oauth";
    }
  ...
  if ((FREE_PROVIDERS as any)[providerId]) return "free";
  if ((OAUTH_PROVIDERS as any)[providerId]) return "oauth";
  if ((APIKEY_PROVIDERS as any)[providerId]) return "apikey";
  if (
