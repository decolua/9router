...
async function normalizeProxyPoolId(proxyPoolId: any): Promise<any> {
...
export async function POST(request: Request): Promise<NextResponse> {
  try {
    ...
    // Validation
    const isValidProvider = (APIKEY_PROVIDERS as any)[provider] ||
      (FREE_TIER_PROVIDERS as any)[provider] ||
    ...
    let providerSpecificData: any = null;
    ...
    const mergedProviderSpecificData: any = {
      ...(providerSpecificData || {}),
    ...
    const newConnection: ProviderConnection = await createProviderConnection({
      provider,
      authType: "apikey",
      name,
      apiKey,
    ...
    const result: any = { ...newConnection };
