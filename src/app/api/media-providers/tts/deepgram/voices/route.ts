...
export async function GET(request: NextRequest) {
  try {
    ...
    const data = await res.json() as { tts?: { languages?: string[]; canonical_name?: string; name?: string; metadata?: { tags?: string[] } }[] };
    ...
    for (const m of ttsModels) {
      ...
      const langs: string[] = Array.isArray(m.languages) && m.languages.length > 0
        ? m.languages
