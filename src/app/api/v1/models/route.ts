import { PROVIDER_MODELS, PROVIDER_ID_TO_ALIAS } from "@/lib/open-sse/config/providerModels";
import { getProviderAlias, isAnthropicCompatibleProvider, isOpenAICompatibleProvider } from "@/shared/constants/providers";
import { getProviderConnections, getCombos, type ProviderConnection } from "@/lib/localDb";
...
async function fetchCompatibleModelIds(connection: ProviderConnection): Promise<string[]> {
...
export async function GET(): Promise<Response> {
  try {
    ...
    const activeConnectionByProvider = new Map<string, ProviderConnection>();
    ...
    if (connections.length === 0) {
      ...
      for (const [providerId, conn] of activeConnectionByProvider.entries()) {
        const staticAlias = (PROVIDER_ID_TO_ALIAS as Record<string, string>)[providerId] || providerId;
        const outputAlias = (
