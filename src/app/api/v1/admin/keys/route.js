...
export async function POST(request) {
  try {
    ...
    if (Array.isArray(body.allowedModels)) opts.allowedModels = body.allowedModels;
    if (Array.isArray(body.allowedProviders)) opts.allowedProviders = body.allowedProviders;
    if (body.monthlyTokenLimit !== undefined) opts.monthlyTokenLimit = body.monthlyTokenLimit;
    if (body.monthlyBudgetUsd !== undefined) opts.monthlyBudgetUsd = body.monthlyBudgetUsd;
    const machineId = await getConsistentMachineId();
    const apiKey = await createApiKey(body.name, machineId, opts);
    // Bootstrap rule may force role=admin regardless of opts; return the record as-created.
