...
export async function GET(request: NextRequest) {
  ...
  const data = await listFiles(purposeParam != null ? { purpose: purposeParam } : {});
...
export async function POST(request: NextRequest) {
  ...
  if (typeof file === "string" || !(file instanceof Blob))
    return errorResponse(
      ...
      "field 'file' must be a file upload",
    );
  if (!purpose)
    return errorResponse(HTTP_STATUS.BAD_REQUEST, "Missing required field: purpose");
  if (typeof purpose !== "string")
    return errorResponse(HTTP_STATUS.BAD_REQUEST, "field 'purpose' must be a string");
