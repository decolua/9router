...
const VALID_TYPES = ["http", "vercel", "cloudflare"] as const;
type PoolType = typeof VALID_TYPES[number];
...
function normalizeProxyPoolUpdate(body: UpdateBody) {
  ...
  if (Object.prototype.hasOwnProperty.call(body, "type")) {
    updates["type"] = VALID_TYPES.includes(body.type as PoolType) ? (body.type as PoolType) : "http";
  }
...
function countBoundConnections(connections: ProviderConnection[], proxyPoolId: string) {
  return connections.filter((connection) => {
    ...
    if (psd === null || typeof psd !== "object") return false;
    return (psd as JsonObject)["proxyPoolId"] === proxyPoolId;
  }).length;
}
  ...
  try {
    ...
    if (boundConnectionCount > 0) {
      return NextResponse.json(
        { error: "Proxy pool is currently in use", boundConnectionCount },
        { status: 409 },
      );
    }
