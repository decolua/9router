...
export async function POST(request: NextRequest, context: { params: Promise<{}> }) {
  ...
  try {
    const body = await request.json() as DenoDeployBody;
    ...
    const app = await createAppRes.json() as DenoApp;
    ...
    const revision = await deployRes.json() as DenoRevision;
    ...
    while (status === "queued" || status === "building") {
      ...
      const statusData = await statusRes.json() as DenoRevision;
