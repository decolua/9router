...
const getErrorMessage = (error: unknown) => {
  ...
  if (error instanceof Error && error.message.includes("timeout")) return "Request timeout (>10s) - provider node not responding";
...
export async function POST(request: NextRequest, context: { params: Promise<{}> }) {
  ...
  try {
    const body = await request.json() as ValidateNodeBody;
    ...
    if (type === "custom-embedding") {
      ...
      if (embedRes.ok) {
        const data = await embedRes.json().catch(() => null) as { data?: Array<{ embedding?: number[] }> } | null;
