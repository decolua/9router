...
export async function GET() {
  try {
    ...
    const providerIds = [...new Set(details.map((r: { provider?: string }) => r.provider).filter(Boolean) as string[])].sort();

    const providerNodes = await getProviderNodes();
    const nodeMap: Record<string, string> = {};
    for (const node of providerNodes) {
      nodeMap[node.id] = node.name ?? node.id;
    }

    const providers = providerIds.map((providerId: string) => {
      let name = providerId;
