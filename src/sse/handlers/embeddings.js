  ...
  while (true) {
    ...
    if (!credentials || credentials.allRateLimited) {
      if (credentials?.allRateLimited) {
        ...
        log.warn("EMBEDDINGS", `[${displayName}/${model}] ${errorMsg} (${credentials.retryAfterHuman})`);
        return unavailableResponse(status, `[${provider}/${model}] ${errorMsg}`, credentials.retryAfter, credentials.retryAfterHuman);
      }
      if (excludeConnectionIds.size === 0) {
        log.error("AUTH", `No credentials for provider: ${displayName}`);
        return errorResponse(HTTP_STATUS.BAD_REQUEST, `No credentials for provider: ${displayName}`);
      }
      log.warn("EMBEDDINGS", "No more accounts available", { provider: displayName });
