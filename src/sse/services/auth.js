...
export async function getProviderCredentials(
	...
	try {
		...
		// Filter by key group if applicable
		let allowedConnectionIds = null;
		if (options?.apiKey) {
			const { getApiKeyByValue, getKeyGroupById } =
				await import("@/lib/localDb");
			const keyRecord = await getApiKeyByValue(options.apiKey);
			if (keyRecord?.groupId) {
				const group = await getKeyGroupById(keyRecord.groupId);
				if (group?.allowedConnectionIds?.length > 0) {
					allowedConnectionIds = new Set(group.allowedConnectionIds);
				}
			}
		}
		log.debug(
			"AUTH",
			`Key group filter: ${allowedConnectionIds?.size ?? "none"} allowed connections`,
		);

		const filteredConnections = allowedConnectionIds
			? connections.filter((c) => allowedConnectionIds.has(c.id))
			: connections;

		log.debug(
			"AUTH",
			`${provider} | total connections: ${filteredConnections.length}, excludeIds: ${excludeSet.size > 0 ? [...excludeSet].join(",") : "none"}, model: ${model || "any"}`,
		);
		...
		const availableConnections = filteredConnections.filter((c) => {
		...
		log.debug(
			"AUTH",
			`${provider} | available: ${availableConnections.length}/${filteredConnections.length}`,
		);
		...
		if (availableConnections.length === 0) {
			...
			const lockedConns = filteredConnections.filter((c) =>
			...
			if (earliest) {
				...
				log.warn(
					"AUTH",
					`${provider} | all ${filteredConnections.length} accounts locked for ${model || "all"} (${formatRetryAfter(earliest)}) | lastError=${earliestConn?.lastError?.slice(0, 50)}`,
				);
				return {
					allRateLimited: true,
					retryAfter: earliest,
			...
			log.warn(
				"AUTH",
				`${provider} | all ${filteredConnections.length} accounts unavailable`,
			);
			return null;
		}
